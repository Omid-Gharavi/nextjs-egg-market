...
export default function ProfilePage() {
    ...
    return (
        <div className="p-8 max-[480px]:p-4">
            ...
            <div className="mt-8 flex flex-col gap-8">
                {
                    inputLists.map((list, index) => (
                        <>
                            <div className="mt-3">
                                <p className='font-medium text-default-700'>{list.title}</p>
                                <InputText
                                    disabled={list.readOnly}
                                    className={`${list.icon && 'pr-11'} ${list.readOnly && '!bg-[#EDEDED] placeholder:text-default-900'} ${list.IR && 'text-left pl-8'}`}
                                ...
                                />
                                {
                                    list.addPhone && userProfile.map((value, index) => (<InputText
                                        disabled={true}
                                ...
                                {
                                    list.sheba && userSheba.map((value, index) => (<InputText
                                        disabled={true}
                                        className={`${list.icon && 'pr-11'} ${list.sheba && 'placeholder:text-default-900'} ${list.readOnly && ' placeholder:text-red-500'}`}
                                        key={index + 1}
                                        placeholder={value}
                                        modal={true}
                                        del={true}
                                ...
                                }
                                {
                                    list.add && <div
                                        ...
                                        onClick={() => {
                                            list.addPhone ? setDialogPhone('add') : setDialogPhone('sheba')
                                            document.getElementById("phone_id").showModal()
                                        }}
                                    >
                                        <span className="icon-light-linear-Add text-default-500"></span>
                                        <p className="font-normal text-sm text-default-500">{list.add}</p>
                                    </div>
                                }
                            </div>
                        </>
            ...
            <Button text={'تایید اطلاعات'} type={'bg-primary text-default-900 w-full mt-[27px]'} />
        </div >
    )
}
