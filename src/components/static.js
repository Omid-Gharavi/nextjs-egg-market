
export const cityData = [
  {
    id: 1,
    province: "آذربایجان شرقی",
    cities: [
      "همه شهرهای آذربایجان شرقی",
      "اسکو",
      "اهر",
      "ایلخچی",
      "آبش احمد",
      "آذرشهر",
      "آقکند",
      "باسمنج",
      "بخشایش",
      "بستان آباد",
      "بناب",
      "بناب جدید",
      "تبریز",
      "ترک",
      "ترکمانچای",
      "تسوج",
      "تیکمه داش",
      "جلفا",
      "خاروانا",
      "خامنه",
      "خراجو",
      "خسروشهر",
      "خمارلو",
      "خواجه",
      "دوزدوزان",
      "زرنق",
      "زنوز",
      "سراب",
      "سردرود",
      "سیس",
      "سیه رود",
      "شبستر",
      "شربیان",
      "شرفخانه",
      "شندآباد",
      "شهرجدیدسهند",
      "صوفیان",
      "عجب شیر",
      "قره آغاج",
      "کشکسرای",
      "کلوانق",
      "کلیبر",
      "کوزه کنان",
      "گوگان",
      "لیلان",
      "مراغه",
      "مرند",
      "ملکان",
      "ممقان",
      "مهربان",
      "میانه",
      "نظرکهریزی",
      "وایقان",
      "ورزقان",
      "هادیشهر",
      "هریس",
      "هشترود",
      "هوراند",
      "یامچی",
    ],
  },
  {
    id: 2,
    province: "آذربایجان غربی",
    cities: [
      "همه شهرهای آذربایجان غربی",
      "ارومیه",
      "اشنویه",
      "ایواوغلی",
      "آواجیق",
      "باروق",
      "بازرگان",
      "بوکان",
      "پلدشت",
      "پیرانشهر",
      "تازه شهر",
      "تکاب",
      "چهاربرج",
      "خلیفان",
      "خوی",
      "دیزج دیز",
      "ربط",
      "سردشت",
      "سرو",
      "سلماس",
      "سیلوانه",
      "سیمینه",
      "سیه چشمه",
      "شاهین دژ",
      "شوط",
      "فیرورق",
      "قره ضیاءالدین",
      "قطور",
      "قوشچی",
      "کشاورز",
      "گردکشانه",
      "ماکو",
      "محمدیار",
      "محمودآباد",
      "مهاباد",
      "میاندوآب",
      "میرآباد",
      "نالوس",
      "نقده",
      "نوشین",
    ],
  },
  {
    id: 3,
    province: "اردبیل",
    cities: [
      "همه شهرهای اردبیل",
      "اردبیل",
      "اصلاندوز",
      "آبی بیگلو",
      "بیله سوار",
      "پارس آباد",
      "تازه کند",
      "تازه کندانگوت",
      "جعفرآباد",
      "خلخال",
      "رضی",
      "سرعین",
      "عنبران",
      "فخرآباد",
      "کلور",
      "کوراییم",
      "گرمی",
      "گیوی",
      "لاهرود",
      "مرادلو",
      "مشگین شهر",
      "نمین",
      "نیر",
      "هشتجین",
      "هیر",
    ],
  },
  {
    id: 4,
    province: "اصفهان",
    cities: [
      "همه شهرهای اصفهان",
      "ابریشم",
      "ابوزیدآباد",
      "اردستان",
      "اژیه",
      "اصفهان",
      "افوس",
      "انارک",
      "ایمانشهر",
      "آران وبیدگل",
      "بادرود",
      "باغ بهادران",
      "بافران",
      "برزک",
      "برف انبار",
      "بوئین ومیاندشت",
      "بهاران شهر",
      "بهارستان",
      "پیربکران",
      "تودشک",
      "تیران",
      "جندق",
      "جوزدان",
      "جوشقان وکامو",
      "چادگان",
      "چرمهین",
      "چمگردان",
      "حبیب آباد",
      "حسن آباد",
      "حنا",
      "خالدآباد",
      "خمینی شهر",
      "خوانسار",
      "خور",
      "خوراسگان",
      "خورزوق",
      "داران",
      "دامنه",
      "درچه پیاز",
      "دستگرد",
      "دولت آباد",
      "دهاقان",
      "دهق",
      "دیزیچه",
      "رزوه",
      "رضوانشهر",
      "زاینده رود",
      "زرین شهر",
      "زواره",
      "زیباشهر",
      "سده لنجان",
      "سفیدشهر",
      "سگزی",
      "سمیرم",
      "شاپورآباد",
      "شاهین شهر",
      "شهرضا",
      "طالخونچه",
      "عسگران",
      "علویچه",
      "فرخی",
      "فریدونشهر",
      "فلاورجان",
      "فولادشهر",
      "قمصر",
      "قهجاورستان",
      "قهدریجان",
      "کاشان",
      "کرکوند",
      "کلیشادوسودرجان",
      "کمشچه",
      "کمه",
      "کوشک",
      "کوهپایه",
      "کهریزسنگ",
      "گرگاب",
      "گزبرخوار",
      "گلپایگان",
      "گلدشت",
      "گلشن",
      "گلشهر",
      "گوگد",
      "لای بید",
      "مبارکه",
      "محمدآباد",
      "مشکات",
      "منظریه",
      "مهاباد",
      "میمه",
      "نائین",
      "نجف آباد",
      "نصرآباد",
      "نطنز",
      "نوش آباد",
      "نیاسر",
      "نیک آباد",
      "ورزنه",
      "ورنامخواست",
      "وزوان",
      "ونک",
      "هرند",
    ],
  },
  {
    id: 5,
    province: "البرز",
    cities: [
      "همه شهرهای البرز",
      "اشتهارد",
      "آسارا",
      "تنکمان",
      "چهارباغ",
      "سیف آباد",
      "شهرجدیدهشتگرد",
      "طالقان",
      "کرج",
      "کمال شهر",
      "کوهسار",
      "گرمدره",
      "ماهدشت",
      "محمدشهر",
      "مشکین دشت",
      "نظرآباد",
      "هشتگرد",
    ],
  },
  {
    id: 6,
    province: "ایلام",
    cities: [
      "همه شهرهای ایلام",
      "ارکواز",
      "ایلام",
      "ایوان",
      "آبدانان",
      "آسمان آباد",
      "بدره",
      "پهله",
      "توحید",
      "چوار",
      "دره شهر",
      "دلگشا",
      "دهلران",
      "زرنه",
      "سراب باغ",
      "سرابله",
      "صالح آباد",
      "لومار",
      "مورموری",
      "موسیان",
      "مهران",
      "میمه",
    ],
  },
  {
    id: 7,
    province: "بوشهر",
    cities: [
      "همه شهرهای بوشهر",
      "امام حسن",
      "انارستان",
      "اهرم",
      "آبپخش",
      "آبدان",
      "برازجان",
      "بردخون",
      "بردستان",
      "بندردیر",
      "بندردیلم",
      "بندرریگ",
      "بندرکنگان",
      "بندرگناوه",
      "بنک",
      "بوشهر",
      "تنگ ارم",
      "جم",
      "چغادک",
      "خارک",
      "خورموج",
      "دالکی",
      "دلوار",
      "ریز",
      "سعدآباد",
      "سیراف",
      "شبانکاره",
      "شنبه",
      "عسلویه",
      "کاکی",
      "کلمه",
      "نخل تقی",
      "وحدتیه",
    ],
  },
  {
    id: 8,
    province: "تهران",
    cities: [
      "همه شهرهای تهران",
      "ارجمند",
      "اسلامشهر",
      "اندیشه",
      "آبسرد",
      "آبعلی",
      "باغستان",
      "باقرشهر",
      "بومهن",
      "پاکدشت",
      "پردیس",
      "پیشوا",
      "تجریش",
      "تهران",
      "جوادآباد",
      "چهاردانگه",
      "حسن آباد",
      "دماوند",
      "رباط کریم",
      "رودهن",
      "ری",
      "شاهدشهر",
      "شریف آباد",
      "شهریار",
      "صالح آباد",
      "صباشهر",
      "صفادشت",
      "فردوسیه",
      "فرون آباد",
      "فشم",
      "فیروزکوه",
      "قدس",
      "قرچک",
      "کهریزک",
      "کیلان",
      "گلستان",
      "لواسان",
      "ملارد",
      "نسیم شهر",
      "نصیرآباد",
      "وحیدیه",
      "ورامین",
    ],
  },
  {
    id: 9,
    province: "چهارمحال وبختیاری",
    cities: [
      "همه شهرهای چهارمهال و بختیاری",
      "اردل",
      "آلونی",
      "باباحیدر",
      "بروجن",
      "بلداجی",
      "بن",
      "جونقان",
      "چلگرد",
      "سامان",
      "سفیددشت",
      "سودجان",
      "سورشجان",
      "شلمزار",
      "شهرکرد",
      "طاقانک",
      "فارسان",
      "فرادنبه",
      "فرخ شهر",
      "کیان",
      "گندمان",
      "گهرو",
      "لردگان",
      "مال خلیفه",
      "ناغان",
      "نافچ",
      "نقنه",
      "هفشجان",
    ],
  },
  {
    id: 10,
    province: "خراسان جنوبی",
    cities: [
      "همه شهرهای خراسان جنوبی",
      "ارسک",
      "اسدیه",
      "اسفدن",
      "اسلامیه",
      "آرین شهر",
      "آیسک",
      "بشرویه",
      "بیرجند",
      "حاجی آباد",
      "خضری دشت بیاض",
      "خوسف",
      "زهان",
      "سرایان",
      "سربیشه",
      "سه قلعه",
      "شوسف",
      "طبس مسینا",
      "فردوس",
      "قائن",
      "قهستان",
      "گزیک",
      "محمد شهر",
      "مود",
      "نهبندان",
      "نیمبلوک",
    ],
  },
  {
    id: 11,
    province: "خراسان رضوی",
    cities: [
      "همه شهرهای خراسان رضوی",
      "احمدآبادصولت",
      "انابد",
      "باجگیران",
      "باخرز",
      "بار",
      "بایگ",
      "بجستان",
      "بردسکن",
      "بیدخت",
      "تایباد",
      "تربت جام",
      "تربت حیدریه",
      "جغتای",
      "جنگل",
      "چاپشلو",
      "چکنه",
      "چناران",
      "خرو",
      "خلیل آباد",
      "خواف",
      "داورزن",
      "درگز",
      "درود",
      "دولت آباد",
      "رباط سنگ",
      "رشتخوار",
      "رضویه",
      "روداب",
      "ریوش",
      "سبزوار",
      "سرخس",
      "سفیدسنگ",
      "سلامی",
      "سلطان آباد",
      "سنگان",
      "شادمهر",
      "شاندیز",
      "ششتمد",
      "شهرآباد",
      "شهرزو",
      "صالح آباد",
      "طرقبه",
      "عشق آباد",
      "فرهادگرد",
      "فریمان",
      "فیروزه",
      "فیض آباد",
      "قاسم آباد",
      "قدمگاه",
      "قلندرآباد",
      "قوچان",
      "کاخک",
      "کاریز",
      "کاشمر",
      "کدکن",
      "کلات",
      "کندر",
      "گلمکان",
      "گناباد",
      "لطف آباد",
      "مزدآوند",
      "مشهد",
      "مشهدریزه",
      "ملک آباد",
      "نشتیفان",
      "نصر آباد",
      "نقاب",
      "نوخندان",
      "نیشابور",
      "نیل شهر",
      "همت آباد",
      "یونسی",
    ],
  },
  {
    id: 12,
    province: "خراسان شمالی",
    cities: [
      "همه شهرهای خراسان شمالی",
      "اسفراین",
      "ایور",
      "آشخانه",
      "بجنورد",
      "پیش قلعه",
      "تیتکانلو",
      "جاجرم",
      "حصارگرمخان",
      "درق",
      "راز",
      "سنخواست",
      "شوقان",
      "شیروان",
      "صفی آباد",
      "فاروج",
      "قاضی",
      "گرمه",
      "لوجلی",
    ],
  },
  {
    id: 13,
    province: "خوزستان",
    cities: [
      "همه شهرهای خوزستان",
      "اروندکنار",
      "الوان",
      "امیدیه",
      "اندیمشک",
      "اهواز",
      "ایذه",
      "آبادان",
      "آغاجاری",
      "باغ ملک",
      "بستان",
      "بندرامام خمینی",
      "بندرماهشهر",
      "بهبهان",
      "ترکالکی",
      "جایزان",
      "جنت مکان",
      "چغامیش",
      "چمران",
      "چوئبده",
      "حر",
      "حسینیه",
      "حمزه",
      "حمیدیه",
      "خرمشهر",
      "دارخوین",
      "دزآب",
      "دزفول",
      "دهدز",
      "رامشیر",
      "رامهرمز",
      "رفیع",
      "زهره",
      "سالند",
      "سردشت",
      "سماله",
      "سوسنگرد",
      "شادگان",
      "شاوور",
      "شرافت",
      "شوش",
      "شوشتر",
      "شیبان",
      "صالح شهر",
      "صالح مشطط",
      "صفی آباد",
      "صیدون",
      "قلعه تل",
      "قلعه خواجه",
      "گتوند",
      "گوریه",
      "لالی",
      "مسجدسلیمان",
      "مشراگه",
      "مقاومت",
      "ملاثانی",
      "میانرود",
      "میداود",
      "مینوشهر",
      "ویس",
      "هفتگل",
      "هندیجان",
      "هویزه",
    ],
  },
  {
    id: 14,
    province: "زنجان",
    cities: [
      "همه شهرهای زنجان",
      "ابهر",
      "ارمغانخانه",
      "آب بر",
      "چورزق",
      "حلب",
      "خرمدره",
      "دندی",
      "زرین آباد",
      "زرین رود",
      "زنجان",
      "سجاس",
      "سلطانیه",
      "سهرورد",
      "صائین قلعه",
      "قیدار",
      "گرماب",
      "ماه نشان",
      "هیدج",
    ],
  },
  {
    id: 15,
    province: "سمنان",
    cities: [
      "همه شهرهای سمنان",
      "امیریه",
      "ایوانکی",
      "آرادان",
      "بسطام",
      "بیارجمند",
      "دامغان",
      "درجزین",
      "دیباج",
      "سرخه",
      "سمنان",
      "شاهرود",
      "شهمیرزاد",
      "کلاته خیج",
      "گرمسار",
      "مجن",
      "مهدی شهر",
      "میامی",
    ],
  },
  {
    id: 16,
    province: "سیستان وبلوچستان",
    cities: [
      "همه شهرهای سیستان و بلوچستان",
      "ادیمی",
      "اسپکه",
      "ایرانشهر",
      "بزمان",
      "بمپور",
      "بنت",
      "بنجار",
      "پیشین",
      "جالق",
      "چاه بهار",
      "خاش",
      "دوست محمد",
      "راسک",
      "زابل",
      "زابلی",
      "زاهدان",
      "زرآباد",
      "زهک",
      "سراوان",
      "سرباز",
      "سوران",
      "سیرکان",
      "علی اکبر",
      "فنوج",
      "قصرقند",
      "کنارک",
      "گشت",
      "گلمورتی",
      "محمدان",
      "محمد آباد",
      "محمدی",
      "میرجاوه",
      "نصرت آباد",
      "نگور",
      "نوک آباد",
      "نیک شهر",
      "هیدوج",
    ],
  },
  {
    id: 17,
    province: "فارس",
    cities: [
      "همه شهرهای فارس",
      "اردکان",
      "ارسنجان",
      "استهبان",
      "اسیر",
      "اشکنان",
      "افزر",
      "اقلید",
      "امام شهر",
      "اوز",
      "اهل",
      "ایج",
      "ایزدخواست",
      "آباده",
      "آباده طشک",
      "باب انار",
      "بالاده",
      "بنارویه",
      "بوانات",
      "بهمن",
      "بیرم",
      "بیضا",
      "جنت شهر",
      "جویم",
      "جهرم",
      "حاجی آباد",
      "حسامی",
      "حسن آباد",
      "خانه زنیان",
      "خاوران",
      "خرامه",
      "خشت",
      "خنج",
      "خور",
      "خومه زار",
      "داراب",
      "داریان",
      "دبیران",
      "دژکرد",
      "دوبرجی",
      "دوزه",
      "دهرم",
      "رامجرد",
      "رونیز",
      "زاهدشهر",
      "زرقان",
      "سده",
      "سروستان",
      "سعادت شهر",
      "سورمق",
      "سیدان",
      "ششده",
      "شهر جدید صدرا",
      "شهرپیر",
      "شیراز",
      "صغاد",
      "صفاشهر",
      "علامرودشت",
      "عمادده",
      "فدامی",
      "فراشبند",
      "فسا",
      "فیروزآباد",
      "قادرآباد",
      "قائمیه",
      "قطب آباد",
      "قطرویه",
      "قیر",
      "کارزین",
      "کازرون",
      "کامفیروز",
      "کره ای",
      "کنارتخته",
      "کوار",
      "کوهنجان",
      "گراش",
      "گله دار",
      "لار",
      "لامرد",
      "لپوئی",
      "لطیفی",
      "مبارک آباد",
      "مرودشت",
      "مشکان",
      "مصیری",
      "مهر",
      "میمند",
      "نوبندگان",
      "نوجین",
      "نودان",
      "نورآباد",
      "نی ریز",
      "وراوی",
      "هماشهر",
    ],
  },
  {
    id: 18,
    province: "قزوین",
    cities: [
      "همه شهرهای قزوین",
      "ارداق",
      "اسفرورین",
      "اقبالیه",
      "الوند",
      "آبگرم",
      "آبیک",
      "آوج",
      "بوئین زهرا",
      "بیدستان",
      "تاکستان",
      "خاکعلی",
      "خرمدشت",
      "دانسفهان",
      "رازمیان",
      "سگزآباد",
      "سیردان",
      "شال",
      "شریفیه",
      "ضیاءآباد",
      "قزوین",
      "کوهین",
      "محمدیه",
      "محمودآبادنمونه",
      "معلم کلایه",
      "نرجه",
    ],
  },
  {
    id: 19,
    province: "قم",
    cities: [
      "همه شهرهای قم",
      "جعفریه",
      "دستجرد",
      "سلفچگان",
      "قم",
      "قنوات",
      "کهک",
    ],
  },
  {
    id: 20,
    province: "کردستان",
    cities: [
      "همه شهرهای کردستان",
      "آرمرده",
      "بابارشانی",
      "بانه",
      "بلبان آباد",
      "بوئین سفلی",
      "بیجار",
      "چناره",
      "دزج",
      "دلبران",
      "دهگلان",
      "دیواندره",
      "زرینه",
      "سروآباد",
      "سریش آباد",
      "سقز",
      "سنندج",
      "شویشه",
      "صاحب",
      "قروه",
      "کامیاران",
      "کانی دینار",
      "کانی سور",
      "مریوان",
      "موچش",
      "یاسوکند",
    ],
  },
  {
    id: 21,
    province: "کرمان",
    cities: [
      "همه شهرهای کرمان",
      "اختیارآباد",
      "ارزوئیه",
      "امین شهر",
      "انار",
      "اندوهجرد",
      "باغین",
      "بافت",
      "بردسیر",
      "بروات",
      "بزنجان",
      "بم",
      "بهرمان",
      "پاریز",
      "جبالبارز",
      "جوپار",
      "جوزم",
      "جیرفت",
      "چترود",
      "خاتون آباد",
      "خانوک",
      "خورسند",
      "درب بهشت",
      "دوساری",
      "دهج",
      "رابر",
      "راور",
      "راین",
      "رفسنجان",
      "رودبار",
      "ریحان شهر",
      "زرند",
      "زنگی آباد",
      "زیدآباد",
      "سرچشمه",
      "سیرجان",
      "شهداد",
      "شهربابک",
      "صفائیه",
      "عنبرآباد",
      "فاریاب",
      "فهرج",
      "قلعه گنج",
      "کاظم آباد",
      "کرمان",
      "کشکوئیه",
      "کوهبنان",
      "کهنوج",
      "کیانشهر",
      "گلباف",
      "گلزار",
      "لاله زار",
      "ماهان",
      "محمد آباد",
      "محی آباد",
      "مردهک",
      "منوجان",
      "نجف شهر",
      "نرماشیر",
      "نظام شهر",
      "نگار",
      "نودژ",
      "هجدک",
      "هماشهر",
      "یزدان شهر",
    ],
  },
  {
    id: 22,
    province: "کرمانشاه",
    cities: [
      "همه شهرهای کرمانشاه",
      "ازگله",
      "اسلام آبادغرب",
      "باینگان",
      "بیستون",
      "پاوه",
      "تازه آباد",
      "جوانرود",
      "حمیل",
      "رباط",
      "روانسر",
      "سرپل ذهاب",
      "سرمست",
      "سطر",
      "سنقر",
      "سومار",
      "شاهو",
      "صحنه",
      "قصرشیرین",
      "کرمانشاه",
      "کرندغرب",
      "کنگاور",
      "کوزران",
      "گهواره",
      "گیلانغرب",
      "میان راهان",
      "نودشه",
      "نوسود",
      "هرسین",
      "هلشی",
    ],
  },
  {
    id: 23,
    province: "کهگیلویه وبویراحمد",
    cities: [
      "همه شهرهای کهگیلویه وبویراحمد",
      "باشت",
      "پاتاوه",
      "چرام",
      "چیتاب",
      "دوگنبدان",
      "دهدشت",
      "دیشموک",
      "سوق",
      "سی سخت",
      "قلعه رئیسی",
      "گراب سفلی",
      "لنده",
      "لیکک",
      "مادوان",
      "مارگون",
      "یاسوج",
    ],
  },
  {
    id: 24,
    province: "گلستان",
    cities: [
      "همه شهرهای گلستان",
      "انبارآلوم",
      "اینچه برون",
      "آزادشهر",
      "آق قلا",
      "بندرگز",
      "ترکمن",
      "جلین",
      "خان ببین",
      "دلند",
      "رامیان",
      "سرخنکلاته",
      "سیمین شهر",
      "علی آباد",
      "فاضل آباد",
      "کردکوی",
      "کلاله",
      "گالیکش",
      "گرگان",
      "گمیش تپه",
      "گنبد کاووس",
      "مراوه تپه",
      "مینودشت",
      "نگین شهر",
      "نوده خاندوز",
      "نوکنده",
    ],
  },
  {
    id: 25,
    province: "گیلان",
    cities: [
      "همه شهرهای گیلان",
      "احمدسرگوراب",
      "اسالم",
      "اطاقور",
      "املش",
      "آستارا",
      "آستانه اشرفیه",
      "بازارجمعه",
      "بره سر",
      "بندرانزلی",
      "پره سر",
      "توتکابن",
      "جیرنده",
      "چابکسر",
      "چاف وچمخاله",
      "چوبر",
      "حویق",
      "خشکبیجار",
      "خمام",
      "دیلمان",
      "رانکوه",
      "رحیم آباد",
      "رستم آباد",
      "رشت",
      "رضوانشهر",
      "رودبار",
      "رودبنه",
      "رودسر",
      "سنگر",
      "سیاهکل",
      "شفت",
      "شلمان",
      "صومعه سرا",
      "فومن",
      "کلاچای",
      "کوچصفهان",
      "کومله",
      "کیاشهر",
      "گوراب زرمیخ",
      "لاهیجان",
      "لشت نشاء",
      "لنگرود",
      "لوشان",
      "لولمان",
      "لوندویل",
      "لیسار",
      "ماسال",
      "ماسوله",
      "مرجقل",
      "منجیل",
      "واجارگاه",
      "هشتپر",
    ],
  },
  {
    id: 26,
    province: "لرستان",
    cities: [
      "همه شهرهای لرستان",
      "ازنا",
      "اشترینان",
      "الشتر",
      "الیگودرز",
      "بروجرد",
      "پلدختر",
      "چالانچولان",
      "چغلوندی",
      "چقابل",
      "خرم آباد",
      "درب گنبد",
      "دورود",
      "زاغه",
      "سپیددشت",
      "سراب دوره",
      "شول آباد",
      "فیروز آباد",
      "کونانی",
      "کوهدشت",
      "گراب",
      "معمولان",
      "مؤمن آباد",
      "نور آباد",
      "ویسیان",
      "هفت چشمه",
    ],
  },
  {
    id: 27,
    province: "مازندران",
    cities: [
      "همه شهرهای مازندران",
      "امیرکلا",
      "ایزدشهر",
      "آلاشت",
      "آمل",
      "بابل",
      "بابلسر",
      "بلده",
      "بهشهر",
      "بهنمیر",
      "پل سفید",
      "پول",
      "تنکابن",
      "جویبار",
      "چالوس",
      "چمستان",
      "خرم آباد",
      "خلیل شهر",
      "خوش رودپی",
      "دابودشت",
      "رامسر",
      "رستمکلا",
      "رویان",
      "رینه",
      "زرگر محله",
      "زیرآب",
      "ساری",
      "سرخرود",
      "سلمان شهر",
      "سورک",
      "شیرگاه",
      "شیرود",
      "عباس آباد",
      "فریدونکنار",
      "فریم",
      "قائم شهر",
      "کتالم وسادات شهر",
      "کلارآباد",
      "کلاردشت",
      "کله بست",
      "کوهی خیل",
      "کیاسر",
      "کیاکلا",
      "گتاب",
      "گزنک",
      "گلوگاه",
      "محمود آباد",
      "مرزن آباد",
      "مرزیکلا",
      "نشتارود",
      "نکا",
      "نور",
      "نوشهر",
    ],
  },
  {
    id: 28,
    province: "مرکزی",
    cities: [
      "همه شهرهای مرکزی",
      "اراک",
      "آستانه",
      "آشتیان",
      "پرندک",
      "تفرش",
      "توره",
      "جاورسیان",
      "خشکرود",
      "خمین",
      "خنداب",
      "داودآباد",
      "دلیجان",
      "رازقان",
      "زاویه",
      "ساروق",
      "ساوه",
      "سنجان",
      "شازند",
      "شهرجدیدمهاجران",
      "غرق آباد",
      "فرمهین",
      "قورچی باشی",
      "کرهرود",
      "کمیجان",
      "مأمونیه",
      "محلات",
      "میلاجرد",
      "نراق",
      "نوبران",
      "نیمور",
      "هندودر",
    ],
  },
  {
    id: 29,
    province: "هرمزگان",
    cities: [
      "همه شهرهای هرمزگان",
      "ابوموسی",
      "بستک",
      "بندرجاسک",
      "بندرچارک",
      "بندرعباس",
      "بندرلنگه",
      "بیکاه",
      "پارسیان",
      "تخت",
      "جناح",
      "حاجی آباد",
      "خمیر",
      "درگهان",
      "دهبارز",
      "رویدر",
      "زیارتعلی",
      "سردشت بشاگرد",
      "سرگز",
      "سندرک",
      "سوزا",
      "سیریک",
      "فارغان",
      "فین",
      "قشم",
      "قلعه قاضی",
      "کنگ",
      "کوشکنار",
      "کیش",
      "گوهران",
      "میناب",
      "هرمز",
      "هشتبندی",
    ],
  },
  {
    id: 30,
    province: "همدان",
    cities: [
      "همه شهرهای همدان",
      "ازندریان",
      "اسدآباد",
      "برزول",
      "بهار",
      "تویسرکان",
      "جورقان",
      "جوکار",
      "دمق",
      "رزن",
      "زنگنه",
      "سامن",
      "سرکان",
      "شیرین سو",
      "صالح آباد",
      "فامنین",
      "فرسفج",
      "فیروزان",
      "قروه در جزین",
      "قهاوند",
      "کبودرآهنگ",
      "گل تپه",
      "گیان",
      "لالجین",
      "مریانج",
      "ملایر",
      "نهاوند",
      "همدان",
    ],
  },
  {
    id: 31,
    province: "یزد",
    cities: [
      "همه شهرهای یزد",
      "ابرکوه",
      "احمدآباد",
      "اردکان",
      "اشکذر",
      "بافق",
      "بفروئیه",
      "بهاباد",
      "تفت",
      "حمیدیا",
      "خضرآباد",
      "دیهوک",
      "زارچ",
      "شاهدیه",
      "طبس",
      "عشق آباد",
      "عقدا",
      "مروست",
      "مهردشت",
      "مهریز",
      "میبد",
      "ندوشن",
      "نیر",
      "هرات",
      "یزد",
    ],
  },
];

export const cities = [
  {
    id: 1,
    title: "اسکو",
    slug: "اسکو",
    province_id: 1,
  },
  {
    id: 2,
    title: "اهر",
    slug: "اهر",
    province_id: 1,
  },
  {
    id: 3,
    title: "ایلخچی",
    slug: "ایلخچی",
    province_id: 1,
  },
  {
    id: 4,
    title: "آبش احمد",
    slug: "آبش-احمد",
    province_id: 1,
  },
  {
    id: 5,
    title: "آذرشهر",
    slug: "آذرشهر",
    province_id: 1,
  },
  {
    id: 6,
    title: "آقکند",
    slug: "آقکند",
    province_id: 1,
  },
  {
    id: 7,
    title: "باسمنج",
    slug: "باسمنج",
    province_id: 1,
  },
  {
    id: 8,
    title: "بخشایش",
    slug: "بخشایش",
    province_id: 1,
  },
  {
    id: 9,
    title: "بستان آباد",
    slug: "بستان-آباد",
    province_id: 1,
  },
  {
    id: 10,
    title: "بناب",
    slug: "بناب",
    province_id: 1,
  },
  {
    id: 11,
    title: "بناب جدید",
    slug: "بناب-جدید",
    province_id: 1,
  },
  {
    id: 12,
    title: "تبریز",
    slug: "تبریز",
    province_id: 1,
  },
  {
    id: 13,
    title: "ترک",
    slug: "ترک",
    province_id: 1,
  },
  {
    id: 14,
    title: "ترکمانچای",
    slug: "ترکمانچای",
    province_id: 1,
  },
  {
    id: 15,
    title: "تسوج",
    slug: "تسوج",
    province_id: 1,
  },
  {
    id: 16,
    title: "تیکمه داش",
    slug: "تیکمه-داش",
    province_id: 1,
  },
  {
    id: 17,
    title: "جلفا",
    slug: "جلفا",
    province_id: 1,
  },
  {
    id: 18,
    title: "خاروانا",
    slug: "خاروانا",
    province_id: 1,
  },
  {
    id: 19,
    title: "خامنه",
    slug: "خامنه",
    province_id: 1,
  },
  {
    id: 20,
    title: "خراجو",
    slug: "خراجو",
    province_id: 1,
  },
  {
    id: 21,
    title: "خسروشهر",
    slug: "خسروشهر",
    province_id: 1,
  },
  {
    id: 22,
    title: "خضرلو",
    slug: "خضرلو",
    province_id: 1,
  },
  {
    id: 23,
    title: "خمارلو",
    slug: "خمارلو",
    province_id: 1,
  },
  {
    id: 24,
    title: "خواجه",
    slug: "خواجه",
    province_id: 1,
  },
  {
    id: 25,
    title: "دوزدوزان",
    slug: "دوزدوزان",
    province_id: 1,
  },
  {
    id: 26,
    title: "زرنق",
    slug: "زرنق",
    province_id: 1,
  },
  {
    id: 27,
    title: "زنوز",
    slug: "زنوز",
    province_id: 1,
  },
  {
    id: 28,
    title: "سراب",
    slug: "سراب",
    province_id: 1,
  },
  {
    id: 29,
    title: "سردرود",
    slug: "سردرود",
    province_id: 1,
  },
  {
    id: 30,
    title: "سهند",
    slug: "سهند",
    province_id: 1,
  },
  {
    id: 31,
    title: "سیس",
    slug: "سیس",
    province_id: 1,
  },
  {
    id: 32,
    title: "سیه رود",
    slug: "سیه-رود",
    province_id: 1,
  },
  {
    id: 33,
    title: "شبستر",
    slug: "شبستر",
    province_id: 1,
  },
  {
    id: 34,
    title: "شربیان",
    slug: "شربیان",
    province_id: 1,
  },
  {
    id: 35,
    title: "شرفخانه",
    slug: "شرفخانه",
    province_id: 1,
  },
  {
    id: 36,
    title: "شندآباد",
    slug: "شندآباد",
    province_id: 1,
  },
  {
    id: 37,
    title: "صوفیان",
    slug: "صوفیان",
    province_id: 1,
  },
  {
    id: 38,
    title: "عجب شیر",
    slug: "عجب-شیر",
    province_id: 1,
  },
  {
    id: 39,
    title: "قره آغاج",
    slug: "قره-آغاج",
    province_id: 1,
  },
  {
    id: 40,
    title: "کشکسرای",
    slug: "کشکسرای",
    province_id: 1,
  },
  {
    id: 41,
    title: "کلوانق",
    slug: "کلوانق",
    province_id: 1,
  },
  {
    id: 42,
    title: "کلیبر",
    slug: "کلیبر",
    province_id: 1,
  },
  {
    id: 43,
    title: "کوزه کنان",
    slug: "کوزه-کنان",
    province_id: 1,
  },
  {
    id: 44,
    title: "گوگان",
    slug: "گوگان",
    province_id: 1,
  },
  {
    id: 45,
    title: "لیلان",
    slug: "لیلان",
    province_id: 1,
  },
  {
    id: 46,
    title: "مراغه",
    slug: "مراغه",
    province_id: 1,
  },
  {
    id: 47,
    title: "مرند",
    slug: "مرند",
    province_id: 1,
  },
  {
    id: 48,
    title: "ملکان",
    slug: "ملکان",
    province_id: 1,
  },
  {
    id: 49,
    title: "ملک کیان",
    slug: "ملک-کیان",
    province_id: 1,
  },
  {
    id: 50,
    title: "ممقان",
    slug: "ممقان",
    province_id: 1,
  },
  {
    id: 51,
    title: "مهربان",
    slug: "مهربان",
    province_id: 1,
  },
  {
    id: 52,
    title: "میانه",
    slug: "میانه",
    province_id: 1,
  },
  {
    id: 53,
    title: "نظرکهریزی",
    slug: "نظرکهریزی",
    province_id: 1,
  },
  {
    id: 54,
    title: "هادی شهر",
    slug: "هادی-شهر",
    province_id: 1,
  },
  {
    id: 55,
    title: "هرگلان",
    slug: "هرگلان",
    province_id: 1,
  },
  {
    id: 56,
    title: "هریس",
    slug: "هریس",
    province_id: 1,
  },
  {
    id: 57,
    title: "هشترود",
    slug: "هشترود",
    province_id: 1,
  },
  {
    id: 58,
    title: "هوراند",
    slug: "هوراند",
    province_id: 1,
  },
  {
    id: 59,
    title: "وایقان",
    slug: "وایقان",
    province_id: 1,
  },
  {
    id: 60,
    title: "ورزقان",
    slug: "ورزقان",
    province_id: 1,
  },
  {
    id: 61,
    title: "یامچی",
    slug: "یامچی",
    province_id: 1,
  },
  {
    id: 62,
    title: "ارومیه",
    slug: "ارومیه",
    province_id: 2,
  },
  {
    id: 63,
    title: "اشنویه",
    slug: "اشنویه",
    province_id: 2,
  },
  {
    id: 64,
    title: "ایواوغلی",
    slug: "ایواوغلی",
    province_id: 2,
  },
  {
    id: 65,
    title: "آواجیق",
    slug: "آواجیق",
    province_id: 2,
  },
  {
    id: 66,
    title: "باروق",
    slug: "باروق",
    province_id: 2,
  },
  {
    id: 67,
    title: "بازرگان",
    slug: "بازرگان",
    province_id: 2,
  },
  {
    id: 68,
    title: "بوکان",
    slug: "بوکان",
    province_id: 2,
  },
  {
    id: 69,
    title: "پلدشت",
    slug: "پلدشت",
    province_id: 2,
  },
  {
    id: 70,
    title: "پیرانشهر",
    slug: "پیرانشهر",
    province_id: 2,
  },
  {
    id: 71,
    title: "تازه شهر",
    slug: "تازه-شهر",
    province_id: 2,
  },
  {
    id: 72,
    title: "تکاب",
    slug: "تکاب",
    province_id: 2,
  },
  {
    id: 73,
    title: "چهاربرج",
    slug: "چهاربرج",
    province_id: 2,
  },
  {
    id: 74,
    title: "خوی",
    slug: "خوی",
    province_id: 2,
  },
  {
    id: 75,
    title: "دیزج دیز",
    slug: "دیزج-دیز",
    province_id: 2,
  },
  {
    id: 76,
    title: "ربط",
    slug: "ربط",
    province_id: 2,
  },
  {
    id: 77,
    title: "سردشت",
    slug: "آذربایجان-غربی-سردشت",
    province_id: 2,
  },
  {
    id: 78,
    title: "سرو",
    slug: "سرو",
    province_id: 2,
  },
  {
    id: 79,
    title: "سلماس",
    slug: "سلماس",
    province_id: 2,
  },
  {
    id: 80,
    title: "سیلوانه",
    slug: "سیلوانه",
    province_id: 2,
  },
  {
    id: 81,
    title: "سیمینه",
    slug: "سیمینه",
    province_id: 2,
  },
  {
    id: 82,
    title: "سیه چشمه",
    slug: "سیه-چشمه",
    province_id: 2,
  },
  {
    id: 83,
    title: "شاهین دژ",
    slug: "شاهین-دژ",
    province_id: 2,
  },
  {
    id: 84,
    title: "شوط",
    slug: "شوط",
    province_id: 2,
  },
  {
    id: 85,
    title: "فیرورق",
    slug: "فیرورق",
    province_id: 2,
  },
  {
    id: 86,
    title: "قره ضیاءالدین",
    slug: "قره-ضیاءالدین",
    province_id: 2,
  },
  {
    id: 87,
    title: "قطور",
    slug: "قطور",
    province_id: 2,
  },
  {
    id: 88,
    title: "قوشچی",
    slug: "قوشچی",
    province_id: 2,
  },
  {
    id: 89,
    title: "کشاورز",
    slug: "کشاورز",
    province_id: 2,
  },
  {
    id: 90,
    title: "گردکشانه",
    slug: "گردکشانه",
    province_id: 2,
  },
  {
    id: 91,
    title: "ماکو",
    slug: "ماکو",
    province_id: 2,
  },
  {
    id: 92,
    title: "محمدیار",
    slug: "محمدیار",
    province_id: 2,
  },
  {
    id: 93,
    title: "محمودآباد",
    slug: "آذربایجان-غربی-محمودآباد",
    province_id: 2,
  },
  {
    id: 94,
    title: "مهاباد",
    slug: "آذربایجان-غربی-مهاباد",
    province_id: 2,
  },
  {
    id: 95,
    title: "میاندوآب",
    slug: "میاندوآب",
    province_id: 2,
  },
  {
    id: 96,
    title: "میرآباد",
    slug: "میرآباد",
    province_id: 2,
  },
  {
    id: 97,
    title: "نالوس",
    slug: "نالوس",
    province_id: 2,
  },
  {
    id: 98,
    title: "نقده",
    slug: "نقده",
    province_id: 2,
  },
  {
    id: 99,
    title: "نوشین",
    slug: "نوشین",
    province_id: 2,
  },
  {
    id: 100,
    title: "اردبیل",
    slug: "اردبیل",
    province_id: 3,
  },
  {
    id: 101,
    title: "اصلاندوز",
    slug: "اصلاندوز",
    province_id: 3,
  },
  {
    id: 102,
    title: "آبی بیگلو",
    slug: "آبی-بیگلو",
    province_id: 3,
  },
  {
    id: 103,
    title: "بیله سوار",
    slug: "بیله-سوار",
    province_id: 3,
  },
  {
    id: 104,
    title: "پارس آباد",
    slug: "پارس-آباد",
    province_id: 3,
  },
  {
    id: 105,
    title: "تازه کند",
    slug: "تازه-کند",
    province_id: 3,
  },
  {
    id: 106,
    title: "تازه کندانگوت",
    slug: "تازه-کندانگوت",
    province_id: 3,
  },
  {
    id: 107,
    title: "جعفرآباد",
    slug: "جعفرآباد",
    province_id: 3,
  },
  {
    id: 108,
    title: "خلخال",
    slug: "خلخال",
    province_id: 3,
  },
  {
    id: 109,
    title: "رضی",
    slug: "رضی",
    province_id: 3,
  },
  {
    id: 110,
    title: "سرعین",
    slug: "سرعین",
    province_id: 3,
  },
  {
    id: 111,
    title: "عنبران",
    slug: "عنبران",
    province_id: 3,
  },
  {
    id: 112,
    title: "فخرآباد",
    slug: "فخرآباد",
    province_id: 3,
  },
  {
    id: 113,
    title: "کلور",
    slug: "کلور",
    province_id: 3,
  },
  {
    id: 114,
    title: "کوراییم",
    slug: "کوراییم",
    province_id: 3,
  },
  {
    id: 115,
    title: "گرمی",
    slug: "گرمی",
    province_id: 3,
  },
  {
    id: 116,
    title: "گیوی",
    slug: "گیوی",
    province_id: 3,
  },
  {
    id: 117,
    title: "لاهرود",
    slug: "لاهرود",
    province_id: 3,
  },
  {
    id: 118,
    title: "مشگین شهر",
    slug: "مشگین-شهر",
    province_id: 3,
  },
  {
    id: 119,
    title: "نمین",
    slug: "نمین",
    province_id: 3,
  },
  {
    id: 120,
    title: "نیر",
    slug: "اردبیل-نیر",
    province_id: 3,
  },
  {
    id: 121,
    title: "هشتجین",
    slug: "هشتجین",
    province_id: 3,
  },
  {
    id: 122,
    title: "هیر",
    slug: "هیر",
    province_id: 3,
  },
  {
    id: 123,
    title: "ابریشم",
    slug: "ابریشم",
    province_id: 4,
  },
  {
    id: 124,
    title: "ابوزیدآباد",
    slug: "ابوزیدآباد",
    province_id: 4,
  },
  {
    id: 125,
    title: "اردستان",
    slug: "اردستان",
    province_id: 4,
  },
  {
    id: 126,
    title: "اژیه",
    slug: "اژیه",
    province_id: 4,
  },
  {
    id: 127,
    title: "اصفهان",
    slug: "اصفهان",
    province_id: 4,
  },
  {
    id: 128,
    title: "افوس",
    slug: "افوس",
    province_id: 4,
  },
  {
    id: 129,
    title: "انارک",
    slug: "انارک",
    province_id: 4,
  },
  {
    id: 130,
    title: "ایمانشهر",
    slug: "ایمانشهر",
    province_id: 4,
  },
  {
    id: 131,
    title: "آران وبیدگل",
    slug: "آران-وبیدگل",
    province_id: 4,
  },
  {
    id: 132,
    title: "بادرود",
    slug: "بادرود",
    province_id: 4,
  },
  {
    id: 133,
    title: "باغ بهادران",
    slug: "باغ-بهادران",
    province_id: 4,
  },
  {
    id: 134,
    title: "بافران",
    slug: "بافران",
    province_id: 4,
  },
  {
    id: 135,
    title: "برزک",
    slug: "برزک",
    province_id: 4,
  },
  {
    id: 136,
    title: "برف انبار",
    slug: "برف-انبار",
    province_id: 4,
  },
  {
    id: 137,
    title: "بهاران شهر",
    slug: "بهاران-شهر",
    province_id: 4,
  },
  {
    id: 138,
    title: "بهارستان",
    slug: "بهارستان",
    province_id: 4,
  },
  {
    id: 139,
    title: "بوئین و میاندشت",
    slug: "بوئین-میاندشت",
    province_id: 4,
  },
  {
    id: 140,
    title: "پیربکران",
    slug: "پیربکران",
    province_id: 4,
  },
  {
    id: 141,
    title: "تودشک",
    slug: "تودشک",
    province_id: 4,
  },
  {
    id: 142,
    title: "تیران",
    slug: "تیران",
    province_id: 4,
  },
  {
    id: 143,
    title: "جندق",
    slug: "جندق",
    province_id: 4,
  },
  {
    id: 144,
    title: "جوزدان",
    slug: "جوزدان",
    province_id: 4,
  },
  {
    id: 145,
    title: "جوشقان و کامو",
    slug: "جوشقان-کامو",
    province_id: 4,
  },
  {
    id: 146,
    title: "چادگان",
    slug: "چادگان",
    province_id: 4,
  },
  {
    id: 147,
    title: "چرمهین",
    slug: "چرمهین",
    province_id: 4,
  },
  {
    id: 148,
    title: "چمگردان",
    slug: "چمگردان",
    province_id: 4,
  },
  {
    id: 149,
    title: "حبیب آباد",
    slug: "حبیب-آباد",
    province_id: 4,
  },
  {
    id: 150,
    title: "حسن آباد",
    slug: "اصفهان-حسن-آباد",
    province_id: 4,
  },
  {
    id: 151,
    title: "حنا",
    slug: "حنا",
    province_id: 4,
  },
  {
    id: 152,
    title: "خالدآباد",
    slug: "خالدآباد",
    province_id: 4,
  },
  {
    id: 153,
    title: "خمینی شهر",
    slug: "خمینی-شهر",
    province_id: 4,
  },
  {
    id: 154,
    title: "خوانسار",
    slug: "خوانسار",
    province_id: 4,
  },
  {
    id: 155,
    title: "خور",
    slug: "اصفهان-خور",
    province_id: 4,
  },
  {
    id: 157,
    title: "خورزوق",
    slug: "خورزوق",
    province_id: 4,
  },
  {
    id: 158,
    title: "داران",
    slug: "داران",
    province_id: 4,
  },
  {
    id: 159,
    title: "دامنه",
    slug: "دامنه",
    province_id: 4,
  },
  {
    id: 160,
    title: "درچه",
    slug: "درچه",
    province_id: 4,
  },
  {
    id: 161,
    title: "دستگرد",
    slug: "دستگرد",
    province_id: 4,
  },
  {
    id: 162,
    title: "دهاقان",
    slug: "دهاقان",
    province_id: 4,
  },
  {
    id: 163,
    title: "دهق",
    slug: "دهق",
    province_id: 4,
  },
  {
    id: 164,
    title: "دولت آباد",
    slug: "اصفهان-دولت-آباد",
    province_id: 4,
  },
  {
    id: 165,
    title: "دیزیچه",
    slug: "دیزیچه",
    province_id: 4,
  },
  {
    id: 166,
    title: "رزوه",
    slug: "رزوه",
    province_id: 4,
  },
  {
    id: 167,
    title: "رضوانشهر",
    slug: "اصفهان-رضوانشهر",
    province_id: 4,
  },
  {
    id: 168,
    title: "زاینده رود",
    slug: "زاینده-رود",
    province_id: 4,
  },
  {
    id: 169,
    title: "زرین شهر",
    slug: "زرین-شهر",
    province_id: 4,
  },
  {
    id: 170,
    title: "زواره",
    slug: "زواره",
    province_id: 4,
  },
  {
    id: 171,
    title: "زیباشهر",
    slug: "زیباشهر",
    province_id: 4,
  },
  {
    id: 172,
    title: "سده لنجان",
    slug: "سده-لنجان",
    province_id: 4,
  },
  {
    id: 173,
    title: "سفیدشهر",
    slug: "سفیدشهر",
    province_id: 4,
  },
  {
    id: 174,
    title: "سگزی",
    slug: "سگزی",
    province_id: 4,
  },
  {
    id: 175,
    title: "سمیرم",
    slug: "سمیرم",
    province_id: 4,
  },
  {
    id: 176,
    title: "شاهین شهر",
    slug: "شاهین-شهر",
    province_id: 4,
  },
  {
    id: 177,
    title: "شهرضا",
    slug: "شهرضا",
    province_id: 4,
  },
  {
    id: 178,
    title: "طالخونچه",
    slug: "طالخونچه",
    province_id: 4,
  },
  {
    id: 179,
    title: "عسگران",
    slug: "عسگران",
    province_id: 4,
  },
  {
    id: 180,
    title: "علویجه",
    slug: "علویجه",
    province_id: 4,
  },
  {
    id: 181,
    title: "فرخی",
    slug: "فرخی",
    province_id: 4,
  },
  {
    id: 182,
    title: "فریدونشهر",
    slug: "فریدونشهر",
    province_id: 4,
  },
  {
    id: 183,
    title: "فلاورجان",
    slug: "فلاورجان",
    province_id: 4,
  },
  {
    id: 184,
    title: "فولادشهر",
    slug: "فولادشهر",
    province_id: 4,
  },
  {
    id: 185,
    title: "قمصر",
    slug: "قمصر",
    province_id: 4,
  },
  {
    id: 186,
    title: "قهجاورستان",
    slug: "قهجاورستان",
    province_id: 4,
  },
  {
    id: 187,
    title: "قهدریجان",
    slug: "قهدریجان",
    province_id: 4,
  },
  {
    id: 188,
    title: "کاشان",
    slug: "کاشان",
    province_id: 4,
  },
  {
    id: 189,
    title: "کرکوند",
    slug: "کرکوند",
    province_id: 4,
  },
  {
    id: 190,
    title: "کلیشاد و سودرجان",
    slug: "کلیشاد-سودرجان",
    province_id: 4,
  },
  {
    id: 191,
    title: "کمشچه",
    slug: "کمشچه",
    province_id: 4,
  },
  {
    id: 192,
    title: "کمه",
    slug: "کمه",
    province_id: 4,
  },
  {
    id: 193,
    title: "کهریزسنگ",
    slug: "کهریزسنگ",
    province_id: 4,
  },
  {
    id: 194,
    title: "کوشک",
    slug: "کوشک",
    province_id: 4,
  },
  {
    id: 195,
    title: "کوهپایه",
    slug: "کوهپایه",
    province_id: 4,
  },
  {
    id: 196,
    title: "گرگاب",
    slug: "گرگاب",
    province_id: 4,
  },
  {
    id: 197,
    title: "گزبرخوار",
    slug: "گزبرخوار",
    province_id: 4,
  },
  {
    id: 198,
    title: "گلپایگان",
    slug: "گلپایگان",
    province_id: 4,
  },
  {
    id: 199,
    title: "گلدشت",
    slug: "گلدشت",
    province_id: 4,
  },
  {
    id: 200,
    title: "گلشهر",
    slug: "گلشهر",
    province_id: 4,
  },
  {
    id: 201,
    title: "گوگد",
    slug: "گوگد",
    province_id: 4,
  },
  {
    id: 202,
    title: "لای بید",
    slug: "لای-بید",
    province_id: 4,
  },
  {
    id: 203,
    title: "مبارکه",
    slug: "مبارکه",
    province_id: 4,
  },
  {
    id: 204,
    title: "مجلسی",
    slug: "مجلسی",
    province_id: 4,
  },
  {
    id: 205,
    title: "محمدآباد",
    slug: "اصفهان-محمدآباد",
    province_id: 4,
  },
  {
    id: 206,
    title: "مشکات",
    slug: "مشکات",
    province_id: 4,
  },
  {
    id: 207,
    title: "منظریه",
    slug: "منظریه",
    province_id: 4,
  },
  {
    id: 208,
    title: "مهاباد",
    slug: "اصفهان-مهاباد",
    province_id: 4,
  },
  {
    id: 209,
    title: "میمه",
    slug: "اصفهان-میمه",
    province_id: 4,
  },
  {
    id: 210,
    title: "نائین",
    slug: "نائین",
    province_id: 4,
  },
  {
    id: 211,
    title: "نجف آباد",
    slug: "نجف-آباد",
    province_id: 4,
  },
  {
    id: 212,
    title: "نصرآباد",
    slug: "اصفهان-نصرآباد",
    province_id: 4,
  },
  {
    id: 213,
    title: "نطنز",
    slug: "نطنز",
    province_id: 4,
  },
  {
    id: 214,
    title: "نوش آباد",
    slug: "نوش-آباد",
    province_id: 4,
  },
  {
    id: 215,
    title: "نیاسر",
    slug: "نیاسر",
    province_id: 4,
  },
  {
    id: 216,
    title: "نیک آباد",
    slug: "نیک-آباد",
    province_id: 4,
  },
  {
    id: 217,
    title: "هرند",
    slug: "هرند",
    province_id: 4,
  },
  {
    id: 218,
    title: "ورزنه",
    slug: "ورزنه",
    province_id: 4,
  },
  {
    id: 219,
    title: "ورنامخواست",
    slug: "ورنامخواست",
    province_id: 4,
  },
  {
    id: 220,
    title: "وزوان",
    slug: "وزوان",
    province_id: 4,
  },
  {
    id: 221,
    title: "ونک",
    slug: "ونک",
    province_id: 4,
  },
  {
    id: 222,
    title: "اسارا",
    slug: "اسارا",
    province_id: 5,
  },
  {
    id: 223,
    title: "اشتهارد",
    slug: "اشتهارد",
    province_id: 5,
  },
  {
    id: 224,
    title: "تنکمان",
    slug: "تنکمان",
    province_id: 5,
  },
  {
    id: 225,
    title: "چهارباغ",
    slug: "چهارباغ",
    province_id: 5,
  },
  {
    id: 226,
    title: "سعید آباد",
    slug: "سعید-آباد",
    province_id: 5,
  },
  {
    id: 227,
    title: "شهر جدید هشتگرد",
    slug: "شهر-جدید-هشتگرد",
    province_id: 5,
  },
  {
    id: 228,
    title: "طالقان",
    slug: "طالقان",
    province_id: 5,
  },
  {
    id: 229,
    title: "کرج",
    slug: "کرج",
    province_id: 5,
  },
  {
    id: 230,
    title: "کمال شهر",
    slug: "کمال-شهر",
    province_id: 5,
  },
  {
    id: 231,
    title: "کوهسار",
    slug: "کوهسار",
    province_id: 5,
  },
  {
    id: 232,
    title: "گرمدره",
    slug: "گرمدره",
    province_id: 5,
  },
  {
    id: 233,
    title: "ماهدشت",
    slug: "ماهدشت",
    province_id: 5,
  },
  {
    id: 234,
    title: "محمدشهر",
    slug: "البرز-محمدشهر",
    province_id: 5,
  },
  {
    id: 235,
    title: "مشکین دشت",
    slug: "مشکین-دشت",
    province_id: 5,
  },
  {
    id: 236,
    title: "نظرآباد",
    slug: "نظرآباد",
    province_id: 5,
  },
  {
    id: 237,
    title: "هشتگرد",
    slug: "هشتگرد",
    province_id: 5,
  },
  {
    id: 238,
    title: "ارکواز",
    slug: "ارکواز",
    province_id: 6,
  },
  {
    id: 239,
    title: "ایلام",
    slug: "ایلام",
    province_id: 6,
  },
  {
    id: 240,
    title: "ایوان",
    slug: "ایوان",
    province_id: 6,
  },
  {
    id: 241,
    title: "آبدانان",
    slug: "آبدانان",
    province_id: 6,
  },
  {
    id: 242,
    title: "آسمان آباد",
    slug: "آسمان-آباد",
    province_id: 6,
  },
  {
    id: 243,
    title: "بدره",
    slug: "بدره",
    province_id: 6,
  },
  {
    id: 244,
    title: "پهله",
    slug: "پهله",
    province_id: 6,
  },
  {
    id: 245,
    title: "توحید",
    slug: "توحید",
    province_id: 6,
  },
  {
    id: 246,
    title: "چوار",
    slug: "چوار",
    province_id: 6,
  },
  {
    id: 247,
    title: "دره شهر",
    slug: "دره-شهر",
    province_id: 6,
  },
  {
    id: 248,
    title: "دلگشا",
    slug: "دلگشا",
    province_id: 6,
  },
  {
    id: 249,
    title: "دهلران",
    slug: "دهلران",
    province_id: 6,
  },
  {
    id: 250,
    title: "زرنه",
    slug: "زرنه",
    province_id: 6,
  },
  {
    id: 251,
    title: "سراب باغ",
    slug: "سراب-باغ",
    province_id: 6,
  },
  {
    id: 252,
    title: "سرابله",
    slug: "سرابله",
    province_id: 6,
  },
  {
    id: 253,
    title: "صالح آباد",
    slug: "ایلام-صالح-آباد",
    province_id: 6,
  },
  {
    id: 254,
    title: "لومار",
    slug: "لومار",
    province_id: 6,
  },
  {
    id: 255,
    title: "مهران",
    slug: "مهران",
    province_id: 6,
  },
  {
    id: 256,
    title: "مورموری",
    slug: "مورموری",
    province_id: 6,
  },
  {
    id: 257,
    title: "موسیان",
    slug: "موسیان",
    province_id: 6,
  },
  {
    id: 258,
    title: "میمه",
    slug: "ایلام-میمه",
    province_id: 6,
  },
  {
    id: 259,
    title: "امام حسن",
    slug: "امام-حسن",
    province_id: 7,
  },
  {
    id: 260,
    title: "انارستان",
    slug: "انارستان",
    province_id: 7,
  },
  {
    id: 261,
    title: "اهرم",
    slug: "اهرم",
    province_id: 7,
  },
  {
    id: 262,
    title: "آب پخش",
    slug: "آب-پخش",
    province_id: 7,
  },
  {
    id: 263,
    title: "آبدان",
    slug: "آبدان",
    province_id: 7,
  },
  {
    id: 264,
    title: "برازجان",
    slug: "برازجان",
    province_id: 7,
  },
  {
    id: 265,
    title: "بردخون",
    slug: "بردخون",
    province_id: 7,
  },
  {
    id: 266,
    title: "بندردیر",
    slug: "بندردیر",
    province_id: 7,
  },
  {
    id: 267,
    title: "بندردیلم",
    slug: "بندردیلم",
    province_id: 7,
  },
  {
    id: 268,
    title: "بندرریگ",
    slug: "بندرریگ",
    province_id: 7,
  },
  {
    id: 269,
    title: "بندرکنگان",
    slug: "بندرکنگان",
    province_id: 7,
  },
  {
    id: 270,
    title: "بندرگناوه",
    slug: "بندرگناوه",
    province_id: 7,
  },
  {
    id: 271,
    title: "بنک",
    slug: "بنک",
    province_id: 7,
  },
  {
    id: 272,
    title: "بوشهر",
    slug: "بوشهر",
    province_id: 7,
  },
  {
    id: 273,
    title: "تنگ ارم",
    slug: "تنگ-ارم",
    province_id: 7,
  },
  {
    id: 274,
    title: "جم",
    slug: "جم",
    province_id: 7,
  },
  {
    id: 275,
    title: "چغادک",
    slug: "چغادک",
    province_id: 7,
  },
  {
    id: 276,
    title: "خارک",
    slug: "خارک",
    province_id: 7,
  },
  {
    id: 277,
    title: "خورموج",
    slug: "خورموج",
    province_id: 7,
  },
  {
    id: 278,
    title: "دالکی",
    slug: "دالکی",
    province_id: 7,
  },
  {
    id: 279,
    title: "دلوار",
    slug: "دلوار",
    province_id: 7,
  },
  {
    id: 280,
    title: "ریز",
    slug: "ریز",
    province_id: 7,
  },
  {
    id: 281,
    title: "سعدآباد",
    slug: "سعدآباد",
    province_id: 7,
  },
  {
    id: 282,
    title: "سیراف",
    slug: "سیراف",
    province_id: 7,
  },
  {
    id: 283,
    title: "شبانکاره",
    slug: "شبانکاره",
    province_id: 7,
  },
  {
    id: 284,
    title: "شنبه",
    slug: "شنبه",
    province_id: 7,
  },
  {
    id: 285,
    title: "عسلویه",
    slug: "عسلویه",
    province_id: 7,
  },
  {
    id: 286,
    title: "کاکی",
    slug: "کاکی",
    province_id: 7,
  },
  {
    id: 287,
    title: "کلمه",
    slug: "کلمه",
    province_id: 7,
  },
  {
    id: 288,
    title: "نخل تقی",
    slug: "نخل-تقی",
    province_id: 7,
  },
  {
    id: 289,
    title: "وحدتیه",
    slug: "وحدتیه",
    province_id: 7,
  },
  {
    id: 290,
    title: "ارجمند",
    slug: "ارجمند",
    province_id: 8,
  },
  {
    id: 291,
    title: "اسلامشهر",
    slug: "اسلامشهر",
    province_id: 8,
  },
  {
    id: 292,
    title: "اندیشه",
    slug: "اندیشه",
    province_id: 8,
  },
  {
    id: 293,
    title: "آبسرد",
    slug: "آبسرد",
    province_id: 8,
  },
  {
    id: 294,
    title: "آبعلی",
    slug: "آبعلی",
    province_id: 8,
  },
  {
    id: 295,
    title: "باغستان",
    slug: "باغستان",
    province_id: 8,
  },
  {
    id: 296,
    title: "باقرشهر",
    slug: "باقرشهر",
    province_id: 8,
  },
  {
    id: 297,
    title: "بومهن",
    slug: "بومهن",
    province_id: 8,
  },
  {
    id: 298,
    title: "پاکدشت",
    slug: "پاکدشت",
    province_id: 8,
  },
  {
    id: 299,
    title: "پردیس",
    slug: "پردیس",
    province_id: 8,
  },
  {
    id: 300,
    title: "پیشوا",
    slug: "پیشوا",
    province_id: 8,
  },
  {
    id: 301,
    title: "تهران",
    slug: "تهران",
    province_id: 8,
  },
  {
    id: 302,
    title: "جوادآباد",
    slug: "جوادآباد",
    province_id: 8,
  },
  {
    id: 303,
    title: "چهاردانگه",
    slug: "چهاردانگه",
    province_id: 8,
  },
  {
    id: 304,
    title: "حسن آباد",
    slug: "تهران-حسن-آباد",
    province_id: 8,
  },
  {
    id: 305,
    title: "دماوند",
    slug: "دماوند",
    province_id: 8,
  },
  {
    id: 306,
    title: "دیزین",
    slug: "دیزین",
    province_id: 8,
  },
  {
    id: 307,
    title: "شهر ری",
    slug: "شهر-ری",
    province_id: 8,
  },
  {
    id: 308,
    title: "رباط کریم",
    slug: "رباط-کریم",
    province_id: 8,
  },
  {
    id: 309,
    title: "رودهن",
    slug: "رودهن",
    province_id: 8,
  },
  {
    id: 310,
    title: "شاهدشهر",
    slug: "شاهدشهر",
    province_id: 8,
  },
  {
    id: 311,
    title: "شریف آباد",
    slug: "شریف-آباد",
    province_id: 8,
  },
  {
    id: 312,
    title: "شمشک",
    slug: "شمشک",
    province_id: 8,
  },
  {
    id: 313,
    title: "شهریار",
    slug: "شهریار",
    province_id: 8,
  },
  {
    id: 314,
    title: "صالح آباد",
    slug: "تهران-صالح-آباد",
    province_id: 8,
  },
  {
    id: 315,
    title: "صباشهر",
    slug: "صباشهر",
    province_id: 8,
  },
  {
    id: 316,
    title: "صفادشت",
    slug: "صفادشت",
    province_id: 8,
  },
  {
    id: 317,
    title: "فردوسیه",
    slug: "فردوسیه",
    province_id: 8,
  },
  {
    id: 318,
    title: "فشم",
    slug: "فشم",
    province_id: 8,
  },
  {
    id: 319,
    title: "فیروزکوه",
    slug: "فیروزکوه",
    province_id: 8,
  },
  {
    id: 320,
    title: "قدس",
    slug: "قدس",
    province_id: 8,
  },
  {
    id: 321,
    title: "قرچک",
    slug: "قرچک",
    province_id: 8,
  },
  {
    id: 322,
    title: "کهریزک",
    slug: "کهریزک",
    province_id: 8,
  },
  {
    id: 323,
    title: "کیلان",
    slug: "کیلان",
    province_id: 8,
  },
  {
    id: 324,
    title: "گلستان",
    slug: "شهر-گلستان",
    province_id: 8,
  },
  {
    id: 325,
    title: "لواسان",
    slug: "لواسان",
    province_id: 8,
  },
  {
    id: 326,
    title: "ملارد",
    slug: "ملارد",
    province_id: 8,
  },
  {
    id: 327,
    title: "میگون",
    slug: "میگون",
    province_id: 8,
  },
  {
    id: 328,
    title: "نسیم شهر",
    slug: "نسیم-شهر",
    province_id: 8,
  },
  {
    id: 329,
    title: "نصیرآباد",
    slug: "نصیرآباد",
    province_id: 8,
  },
  {
    id: 330,
    title: "وحیدیه",
    slug: "وحیدیه",
    province_id: 8,
  },
  {
    id: 331,
    title: "ورامین",
    slug: "ورامین",
    province_id: 8,
  },
  {
    id: 332,
    title: "اردل",
    slug: "اردل",
    province_id: 9,
  },
  {
    id: 333,
    title: "آلونی",
    slug: "آلونی",
    province_id: 9,
  },
  {
    id: 334,
    title: "باباحیدر",
    slug: "باباحیدر",
    province_id: 9,
  },
  {
    id: 335,
    title: "بروجن",
    slug: "بروجن",
    province_id: 9,
  },
  {
    id: 336,
    title: "بلداجی",
    slug: "بلداجی",
    province_id: 9,
  },
  {
    id: 337,
    title: "بن",
    slug: "بن",
    province_id: 9,
  },
  {
    id: 338,
    title: "جونقان",
    slug: "جونقان",
    province_id: 9,
  },
  {
    id: 339,
    title: "چلگرد",
    slug: "چلگرد",
    province_id: 9,
  },
  {
    id: 340,
    title: "سامان",
    slug: "سامان",
    province_id: 9,
  },
  {
    id: 341,
    title: "سفیددشت",
    slug: "سفیددشت",
    province_id: 9,
  },
  {
    id: 342,
    title: "سودجان",
    slug: "سودجان",
    province_id: 9,
  },
  {
    id: 343,
    title: "سورشجان",
    slug: "سورشجان",
    province_id: 9,
  },
  {
    id: 344,
    title: "شلمزار",
    slug: "شلمزار",
    province_id: 9,
  },
  {
    id: 345,
    title: "شهرکرد",
    slug: "شهرکرد",
    province_id: 9,
  },
  {
    id: 346,
    title: "طاقانک",
    slug: "طاقانک",
    province_id: 9,
  },
  {
    id: 347,
    title: "فارسان",
    slug: "فارسان",
    province_id: 9,
  },
  {
    id: 348,
    title: "فرادنبه",
    slug: "فرادبنه",
    province_id: 9,
  },
  {
    id: 349,
    title: "فرخ شهر",
    slug: "فرخ-شهر",
    province_id: 9,
  },
  {
    id: 350,
    title: "کیان",
    slug: "کیان",
    province_id: 9,
  },
  {
    id: 351,
    title: "گندمان",
    slug: "گندمان",
    province_id: 9,
  },
  {
    id: 352,
    title: "گهرو",
    slug: "گهرو",
    province_id: 9,
  },
  {
    id: 353,
    title: "لردگان",
    slug: "لردگان",
    province_id: 9,
  },
  {
    id: 354,
    title: "مال خلیفه",
    slug: "مال-خلیفه",
    province_id: 9,
  },
  {
    id: 355,
    title: "ناغان",
    slug: "ناغان",
    province_id: 9,
  },
  {
    id: 356,
    title: "نافچ",
    slug: "نافچ",
    province_id: 9,
  },
  {
    id: 357,
    title: "نقنه",
    slug: "نقنه",
    province_id: 9,
  },
  {
    id: 358,
    title: "هفشجان",
    slug: "هفشجان",
    province_id: 9,
  },
  {
    id: 359,
    title: "ارسک",
    slug: "ارسک",
    province_id: 10,
  },
  {
    id: 360,
    title: "اسدیه",
    slug: "اسدیه",
    province_id: 10,
  },
  {
    id: 361,
    title: "اسفدن",
    slug: "اسفدن",
    province_id: 10,
  },
  {
    id: 362,
    title: "اسلامیه",
    slug: "اسلامیه",
    province_id: 10,
  },
  {
    id: 363,
    title: "آرین شهر",
    slug: "آرین-شهر",
    province_id: 10,
  },
  {
    id: 364,
    title: "آیسک",
    slug: "آیسک",
    province_id: 10,
  },
  {
    id: 365,
    title: "بشرویه",
    slug: "بشرویه",
    province_id: 10,
  },
  {
    id: 366,
    title: "بیرجند",
    slug: "بیرجند",
    province_id: 10,
  },
  {
    id: 367,
    title: "حاجی آباد",
    slug: "خراسان-جنوبی-حاجی-آباد",
    province_id: 10,
  },
  {
    id: 368,
    title: "خضری دشت بیاض",
    slug: "خضری-دشت-بیاض",
    province_id: 10,
  },
  {
    id: 369,
    title: "خوسف",
    slug: "خوسف",
    province_id: 10,
  },
  {
    id: 370,
    title: "زهان",
    slug: "زهان",
    province_id: 10,
  },
  {
    id: 371,
    title: "سرایان",
    slug: "سرایان",
    province_id: 10,
  },
  {
    id: 372,
    title: "سربیشه",
    slug: "سربیشه",
    province_id: 10,
  },
  {
    id: 373,
    title: "سه قلعه",
    slug: "سه-قلعه",
    province_id: 10,
  },
  {
    id: 374,
    title: "شوسف",
    slug: "شوسف",
    province_id: 10,
  },
  {
    id: 375,
    title: "طبس ",
    slug: "خراسان-جنوبی-طبس-",
    province_id: 10,
  },
  {
    id: 376,
    title: "فردوس",
    slug: "فردوس",
    province_id: 10,
  },
  {
    id: 377,
    title: "قاین",
    slug: "قاین",
    province_id: 10,
  },
  {
    id: 378,
    title: "قهستان",
    slug: "قهستان",
    province_id: 10,
  },
  {
    id: 379,
    title: "محمدشهر",
    slug: "خراسان-جنوبی-محمدشهر",
    province_id: 10,
  },
  {
    id: 380,
    title: "مود",
    slug: "مود",
    province_id: 10,
  },
  {
    id: 381,
    title: "نهبندان",
    slug: "نهبندان",
    province_id: 10,
  },
  {
    id: 382,
    title: "نیمبلوک",
    slug: "نیمبلوک",
    province_id: 10,
  },
  {
    id: 383,
    title: "احمدآباد صولت",
    slug: "احمدآباد-صولت",
    province_id: 11,
  },
  {
    id: 384,
    title: "انابد",
    slug: "انابد",
    province_id: 11,
  },
  {
    id: 385,
    title: "باجگیران",
    slug: "باجگیران",
    province_id: 11,
  },
  {
    id: 386,
    title: "باخرز",
    slug: "باخرز",
    province_id: 11,
  },
  {
    id: 387,
    title: "بار",
    slug: "بار",
    province_id: 11,
  },
  {
    id: 388,
    title: "بایگ",
    slug: "بایگ",
    province_id: 11,
  },
  {
    id: 389,
    title: "بجستان",
    slug: "بجستان",
    province_id: 11,
  },
  {
    id: 390,
    title: "بردسکن",
    slug: "بردسکن",
    province_id: 11,
  },
  {
    id: 391,
    title: "بیدخت",
    slug: "بیدخت",
    province_id: 11,
  },
  {
    id: 392,
    title: "تایباد",
    slug: "تایباد",
    province_id: 11,
  },
  {
    id: 393,
    title: "تربت جام",
    slug: "تربت-جام",
    province_id: 11,
  },
  {
    id: 394,
    title: "تربت حیدریه",
    slug: "تربت-حیدریه",
    province_id: 11,
  },
  {
    id: 395,
    title: "جغتای",
    slug: "جغتای",
    province_id: 11,
  },
  {
    id: 396,
    title: "جنگل",
    slug: "جنگل",
    province_id: 11,
  },
  {
    id: 397,
    title: "چاپشلو",
    slug: "چاپشلو",
    province_id: 11,
  },
  {
    id: 398,
    title: "چکنه",
    slug: "چکنه",
    province_id: 11,
  },
  {
    id: 399,
    title: "چناران",
    slug: "چناران",
    province_id: 11,
  },
  {
    id: 400,
    title: "خرو",
    slug: "خرو",
    province_id: 11,
  },
  {
    id: 401,
    title: "خلیل آباد",
    slug: "خلیل-آباد",
    province_id: 11,
  },
  {
    id: 402,
    title: "خواف",
    slug: "خواف",
    province_id: 11,
  },
  {
    id: 403,
    title: "داورزن",
    slug: "داورزن",
    province_id: 11,
  },
  {
    id: 404,
    title: "درگز",
    slug: "درگز",
    province_id: 11,
  },
  {
    id: 405,
    title: "در رود",
    slug: "در-رود",
    province_id: 11,
  },
  {
    id: 406,
    title: "دولت آباد",
    slug: "خراسان-رضوی-دولت-آباد",
    province_id: 11,
  },
  {
    id: 407,
    title: "رباط سنگ",
    slug: "رباط-سنگ",
    province_id: 11,
  },
  {
    id: 408,
    title: "رشتخوار",
    slug: "رشتخوار",
    province_id: 11,
  },
  {
    id: 409,
    title: "رضویه",
    slug: "رضویه",
    province_id: 11,
  },
  {
    id: 410,
    title: "روداب",
    slug: "روداب",
    province_id: 11,
  },
  {
    id: 411,
    title: "ریوش",
    slug: "ریوش",
    province_id: 11,
  },
  {
    id: 412,
    title: "سبزوار",
    slug: "سبزوار",
    province_id: 11,
  },
  {
    id: 413,
    title: "سرخس",
    slug: "سرخس",
    province_id: 11,
  },
  {
    id: 414,
    title: "سفیدسنگ",
    slug: "سفیدسنگ",
    province_id: 11,
  },
  {
    id: 415,
    title: "سلامی",
    slug: "سلامی",
    province_id: 11,
  },
  {
    id: 416,
    title: "سلطان آباد",
    slug: "سلطان-آباد",
    province_id: 11,
  },
  {
    id: 417,
    title: "سنگان",
    slug: "سنگان",
    province_id: 11,
  },
  {
    id: 418,
    title: "شادمهر",
    slug: "شادمهر",
    province_id: 11,
  },
  {
    id: 419,
    title: "شاندیز",
    slug: "شاندیز",
    province_id: 11,
  },
  {
    id: 420,
    title: "ششتمد",
    slug: "ششتمد",
    province_id: 11,
  },
  {
    id: 421,
    title: "شهرآباد",
    slug: "شهرآباد",
    province_id: 11,
  },
  {
    id: 422,
    title: "شهرزو",
    slug: "شهرزو",
    province_id: 11,
  },
  {
    id: 423,
    title: "صالح آباد",
    slug: "خراسان-رضوی-صالح-آباد",
    province_id: 11,
  },
  {
    id: 424,
    title: "طرقبه",
    slug: "طرقبه",
    province_id: 11,
  },
  {
    id: 425,
    title: "عشق آباد",
    slug: "خراسان-رضوی-عشق-آباد",
    province_id: 11,
  },
  {
    id: 426,
    title: "فرهادگرد",
    slug: "فرهادگرد",
    province_id: 11,
  },
  {
    id: 427,
    title: "فریمان",
    slug: "فریمان",
    province_id: 11,
  },
  {
    id: 428,
    title: "فیروزه",
    slug: "فیروزه",
    province_id: 11,
  },
  {
    id: 429,
    title: "فیض آباد",
    slug: "فیض-آباد",
    province_id: 11,
  },
  {
    id: 430,
    title: "قاسم آباد",
    slug: "قاسم-آباد",
    province_id: 11,
  },
  {
    id: 431,
    title: "قدمگاه",
    slug: "قدمگاه",
    province_id: 11,
  },
  {
    id: 432,
    title: "قلندرآباد",
    slug: "قلندرآباد",
    province_id: 11,
  },
  {
    id: 433,
    title: "قوچان",
    slug: "قوچان",
    province_id: 11,
  },
  {
    id: 434,
    title: "کاخک",
    slug: "کاخک",
    province_id: 11,
  },
  {
    id: 435,
    title: "کاریز",
    slug: "کاریز",
    province_id: 11,
  },
  {
    id: 436,
    title: "کاشمر",
    slug: "کاشمر",
    province_id: 11,
  },
  {
    id: 437,
    title: "کدکن",
    slug: "کدکن",
    province_id: 11,
  },
  {
    id: 438,
    title: "کلات",
    slug: "کلات",
    province_id: 11,
  },
  {
    id: 439,
    title: "کندر",
    slug: "کندر",
    province_id: 11,
  },
  {
    id: 440,
    title: "گلمکان",
    slug: "گلمکان",
    province_id: 11,
  },
  {
    id: 441,
    title: "گناباد",
    slug: "گناباد",
    province_id: 11,
  },
  {
    id: 442,
    title: "لطف آباد",
    slug: "لطف-آباد",
    province_id: 11,
  },
  {
    id: 443,
    title: "مزدآوند",
    slug: "مزدآوند",
    province_id: 11,
  },
  {
    id: 444,
    title: "مشهد",
    slug: "مشهد",
    province_id: 11,
  },
  {
    id: 445,
    title: "ملک آباد",
    slug: "ملک-آباد",
    province_id: 11,
  },
  {
    id: 446,
    title: "نشتیفان",
    slug: "نشتیفان",
    province_id: 11,
  },
  {
    id: 447,
    title: "نصرآباد",
    slug: "خراسان-رضوی-نصرآباد",
    province_id: 11,
  },
  {
    id: 448,
    title: "نقاب",
    slug: "نقاب",
    province_id: 11,
  },
  {
    id: 449,
    title: "نوخندان",
    slug: "نوخندان",
    province_id: 11,
  },
  {
    id: 450,
    title: "نیشابور",
    slug: "نیشابور",
    province_id: 11,
  },
  {
    id: 451,
    title: "نیل شهر",
    slug: "نیل-شهر",
    province_id: 11,
  },
  {
    id: 452,
    title: "همت آباد",
    slug: "همت-آباد",
    province_id: 11,
  },
  {
    id: 453,
    title: "یونسی",
    slug: "یونسی",
    province_id: 11,
  },
  {
    id: 454,
    title: "اسفراین",
    slug: "اسفراین",
    province_id: 12,
  },
  {
    id: 455,
    title: "ایور",
    slug: "ایور",
    province_id: 12,
  },
  {
    id: 456,
    title: "آشخانه",
    slug: "آشخانه",
    province_id: 12,
  },
  {
    id: 457,
    title: "بجنورد",
    slug: "بجنورد",
    province_id: 12,
  },
  {
    id: 458,
    title: "پیش قلعه",
    slug: "پیش-قلعه",
    province_id: 12,
  },
  {
    id: 459,
    title: "تیتکانلو",
    slug: "تیتکانلو",
    province_id: 12,
  },
  {
    id: 460,
    title: "جاجرم",
    slug: "جاجرم",
    province_id: 12,
  },
  {
    id: 461,
    title: "حصارگرمخان",
    slug: "حصارگرمخان",
    province_id: 12,
  },
  {
    id: 462,
    title: "درق",
    slug: "درق",
    province_id: 12,
  },
  {
    id: 463,
    title: "راز",
    slug: "راز",
    province_id: 12,
  },
  {
    id: 464,
    title: "سنخواست",
    slug: "سنخواست",
    province_id: 12,
  },
  {
    id: 465,
    title: "شوقان",
    slug: "شوقان",
    province_id: 12,
  },
  {
    id: 466,
    title: "شیروان",
    slug: "شیروان",
    province_id: 12,
  },
  {
    id: 467,
    title: "صفی آباد",
    slug: "خراسان-شمالی-صفی-آباد",
    province_id: 12,
  },
  {
    id: 468,
    title: "فاروج",
    slug: "فاروج",
    province_id: 12,
  },
  {
    id: 469,
    title: "قاضی",
    slug: "قاضی",
    province_id: 12,
  },
  {
    id: 470,
    title: "گرمه",
    slug: "گرمه",
    province_id: 12,
  },
  {
    id: 471,
    title: "لوجلی",
    slug: "لوجلی",
    province_id: 12,
  },
  {
    id: 472,
    title: "اروندکنار",
    slug: "اروندکنار",
    province_id: 13,
  },
  {
    id: 473,
    title: "الوان",
    slug: "الوان",
    province_id: 13,
  },
  {
    id: 474,
    title: "امیدیه",
    slug: "امیدیه",
    province_id: 13,
  },
  {
    id: 475,
    title: "اندیمشک",
    slug: "اندیمشک",
    province_id: 13,
  },
  {
    id: 476,
    title: "اهواز",
    slug: "اهواز",
    province_id: 13,
  },
  {
    id: 477,
    title: "ایذه",
    slug: "ایذه",
    province_id: 13,
  },
  {
    id: 478,
    title: "آبادان",
    slug: "آبادان",
    province_id: 13,
  },
  {
    id: 479,
    title: "آغاجاری",
    slug: "آغاجاری",
    province_id: 13,
  },
  {
    id: 480,
    title: "باغ ملک",
    slug: "باغ-ملک",
    province_id: 13,
  },
  {
    id: 481,
    title: "بستان",
    slug: "بستان",
    province_id: 13,
  },
  {
    id: 482,
    title: "بندرامام خمینی",
    slug: "بندرامام-خمینی",
    province_id: 13,
  },
  {
    id: 483,
    title: "بندرماهشهر",
    slug: "بندرماهشهر",
    province_id: 13,
  },
  {
    id: 484,
    title: "بهبهان",
    slug: "بهبهان",
    province_id: 13,
  },
  {
    id: 485,
    title: "ترکالکی",
    slug: "ترکالکی",
    province_id: 13,
  },
  {
    id: 486,
    title: "جایزان",
    slug: "جایزان",
    province_id: 13,
  },
  {
    id: 487,
    title: "چمران",
    slug: "چمران",
    province_id: 13,
  },
  {
    id: 488,
    title: "چویبده",
    slug: "چویبده",
    province_id: 13,
  },
  {
    id: 489,
    title: "حر",
    slug: "حر",
    province_id: 13,
  },
  {
    id: 490,
    title: "حسینیه",
    slug: "حسینیه",
    province_id: 13,
  },
  {
    id: 491,
    title: "حمزه",
    slug: "حمزه",
    province_id: 13,
  },
  {
    id: 492,
    title: "حمیدیه",
    slug: "حمیدیه",
    province_id: 13,
  },
  {
    id: 493,
    title: "خرمشهر",
    slug: "خرمشهر",
    province_id: 13,
  },
  {
    id: 494,
    title: "دارخوین",
    slug: "دارخوین",
    province_id: 13,
  },
  {
    id: 495,
    title: "دزآب",
    slug: "دزآب",
    province_id: 13,
  },
  {
    id: 496,
    title: "دزفول",
    slug: "دزفول",
    province_id: 13,
  },
  {
    id: 497,
    title: "دهدز",
    slug: "دهدز",
    province_id: 13,
  },
  {
    id: 498,
    title: "رامشیر",
    slug: "رامشیر",
    province_id: 13,
  },
  {
    id: 499,
    title: "رامهرمز",
    slug: "رامهرمز",
    province_id: 13,
  },
  {
    id: 500,
    title: "رفیع",
    slug: "رفیع",
    province_id: 13,
  },
  {
    id: 501,
    title: "زهره",
    slug: "زهره",
    province_id: 13,
  },
  {
    id: 502,
    title: "سالند",
    slug: "سالند",
    province_id: 13,
  },
  {
    id: 503,
    title: "سردشت",
    slug: "خوزستان-سردشت",
    province_id: 13,
  },
  {
    id: 504,
    title: "سوسنگرد",
    slug: "سوسنگرد",
    province_id: 13,
  },
  {
    id: 505,
    title: "شادگان",
    slug: "شادگان",
    province_id: 13,
  },
  {
    id: 506,
    title: "شاوور",
    slug: "شاوور",
    province_id: 13,
  },
  {
    id: 507,
    title: "شرافت",
    slug: "شرافت",
    province_id: 13,
  },
  {
    id: 508,
    title: "شوش",
    slug: "شوش",
    province_id: 13,
  },
  {
    id: 509,
    title: "شوشتر",
    slug: "شوشتر",
    province_id: 13,
  },
  {
    id: 510,
    title: "شیبان",
    slug: "شیبان",
    province_id: 13,
  },
  {
    id: 511,
    title: "صالح شهر",
    slug: "صالح-شهر",
    province_id: 13,
  },
  {
    id: 512,
    title: "صفی آباد",
    slug: "خوزستان-صفی-آباد",
    province_id: 13,
  },
  {
    id: 513,
    title: "صیدون",
    slug: "صیدون",
    province_id: 13,
  },
  {
    id: 514,
    title: "قلعه تل",
    slug: "قلعه-تل",
    province_id: 13,
  },
  {
    id: 515,
    title: "قلعه خواجه",
    slug: "قلعه-خواجه",
    province_id: 13,
  },
  {
    id: 516,
    title: "گتوند",
    slug: "گتوند",
    province_id: 13,
  },
  {
    id: 517,
    title: "لالی",
    slug: "لالی",
    province_id: 13,
  },
  {
    id: 518,
    title: "مسجدسلیمان",
    slug: "مسجدسلیمان",
    province_id: 13,
  },
  {
    id: 520,
    title: "ملاثانی",
    slug: "ملاثانی",
    province_id: 13,
  },
  {
    id: 521,
    title: "میانرود",
    slug: "میانرود",
    province_id: 13,
  },
  {
    id: 522,
    title: "مینوشهر",
    slug: "مینوشهر",
    province_id: 13,
  },
  {
    id: 523,
    title: "هفتگل",
    slug: "هفتگل",
    province_id: 13,
  },
  {
    id: 524,
    title: "هندیجان",
    slug: "هندیجان",
    province_id: 13,
  },
  {
    id: 525,
    title: "هویزه",
    slug: "هویزه",
    province_id: 13,
  },
  {
    id: 526,
    title: "ویس",
    slug: "ویس",
    province_id: 13,
  },
  {
    id: 527,
    title: "ابهر",
    slug: "ابهر",
    province_id: 14,
  },
  {
    id: 528,
    title: "ارمغان خانه",
    slug: "ارمغان-خانه",
    province_id: 14,
  },
  {
    id: 529,
    title: "آب بر",
    slug: "آب-بر",
    province_id: 14,
  },
  {
    id: 530,
    title: "چورزق",
    slug: "چورزق",
    province_id: 14,
  },
  {
    id: 531,
    title: "حلب",
    slug: "حلب",
    province_id: 14,
  },
  {
    id: 532,
    title: "خرمدره",
    slug: "خرمدره",
    province_id: 14,
  },
  {
    id: 533,
    title: "دندی",
    slug: "دندی",
    province_id: 14,
  },
  {
    id: 534,
    title: "زرین آباد",
    slug: "زرین-آباد",
    province_id: 14,
  },
  {
    id: 535,
    title: "زرین رود",
    slug: "زرین-رود",
    province_id: 14,
  },
  {
    id: 536,
    title: "زنجان",
    slug: "زنجان",
    province_id: 14,
  },
  {
    id: 537,
    title: "سجاس",
    slug: "سجاس",
    province_id: 14,
  },
  {
    id: 538,
    title: "سلطانیه",
    slug: "سلطانیه",
    province_id: 14,
  },
  {
    id: 539,
    title: "سهرورد",
    slug: "سهرورد",
    province_id: 14,
  },
  {
    id: 540,
    title: "صائین قلعه",
    slug: "صائین-قلعه",
    province_id: 14,
  },
  {
    id: 541,
    title: "قیدار",
    slug: "قیدار",
    province_id: 14,
  },
  {
    id: 542,
    title: "گرماب",
    slug: "گرماب",
    province_id: 14,
  },
  {
    id: 543,
    title: "ماه نشان",
    slug: "ماه-نشان",
    province_id: 14,
  },
  {
    id: 544,
    title: "هیدج",
    slug: "هیدج",
    province_id: 14,
  },
  {
    id: 545,
    title: "امیریه",
    slug: "امیریه",
    province_id: 15,
  },
  {
    id: 546,
    title: "ایوانکی",
    slug: "ایوانکی",
    province_id: 15,
  },
  {
    id: 547,
    title: "آرادان",
    slug: "آرادان",
    province_id: 15,
  },
  {
    id: 548,
    title: "بسطام",
    slug: "بسطام",
    province_id: 15,
  },
  {
    id: 549,
    title: "بیارجمند",
    slug: "بیارجمند",
    province_id: 15,
  },
  {
    id: 550,
    title: "دامغان",
    slug: "دامغان",
    province_id: 15,
  },
  {
    id: 551,
    title: "درجزین",
    slug: "درجزین",
    province_id: 15,
  },
  {
    id: 552,
    title: "دیباج",
    slug: "دیباج",
    province_id: 15,
  },
  {
    id: 553,
    title: "سرخه",
    slug: "سرخه",
    province_id: 15,
  },
  {
    id: 554,
    title: "سمنان",
    slug: "سمنان",
    province_id: 15,
  },
  {
    id: 555,
    title: "شاهرود",
    slug: "شاهرود",
    province_id: 15,
  },
  {
    id: 556,
    title: "شهمیرزاد",
    slug: "شهمیرزاد",
    province_id: 15,
  },
  {
    id: 557,
    title: "کلاته خیج",
    slug: "کلاته-خیج",
    province_id: 15,
  },
  {
    id: 558,
    title: "گرمسار",
    slug: "گرمسار",
    province_id: 15,
  },
  {
    id: 559,
    title: "مجن",
    slug: "مجن",
    province_id: 15,
  },
  {
    id: 560,
    title: "مهدی شهر",
    slug: "مهدی-شهر",
    province_id: 15,
  },
  {
    id: 561,
    title: "میامی",
    slug: "میامی",
    province_id: 15,
  },
  {
    id: 562,
    title: "ادیمی",
    slug: "ادیمی",
    province_id: 16,
  },
  {
    id: 563,
    title: "اسپکه",
    slug: "اسپکه",
    province_id: 16,
  },
  {
    id: 564,
    title: "ایرانشهر",
    slug: "ایرانشهر",
    province_id: 16,
  },
  {
    id: 565,
    title: "بزمان",
    slug: "بزمان",
    province_id: 16,
  },
  {
    id: 566,
    title: "بمپور",
    slug: "بمپور",
    province_id: 16,
  },
  {
    id: 567,
    title: "بنت",
    slug: "بنت",
    province_id: 16,
  },
  {
    id: 568,
    title: "بنجار",
    slug: "بنجار",
    province_id: 16,
  },
  {
    id: 569,
    title: "پیشین",
    slug: "پیشین",
    province_id: 16,
  },
  {
    id: 570,
    title: "جالق",
    slug: "جالق",
    province_id: 16,
  },
  {
    id: 571,
    title: "چابهار",
    slug: "چابهار",
    province_id: 16,
  },
  {
    id: 572,
    title: "خاش",
    slug: "خاش",
    province_id: 16,
  },
  {
    id: 573,
    title: "دوست محمد",
    slug: "دوست-محمد",
    province_id: 16,
  },
  {
    id: 574,
    title: "راسک",
    slug: "راسک",
    province_id: 16,
  },
  {
    id: 575,
    title: "زابل",
    slug: "زابل",
    province_id: 16,
  },
  {
    id: 576,
    title: "زابلی",
    slug: "زابلی",
    province_id: 16,
  },
  {
    id: 577,
    title: "زاهدان",
    slug: "زاهدان",
    province_id: 16,
  },
  {
    id: 578,
    title: "زهک",
    slug: "زهک",
    province_id: 16,
  },
  {
    id: 579,
    title: "سراوان",
    slug: "سراوان",
    province_id: 16,
  },
  {
    id: 580,
    title: "سرباز",
    slug: "سرباز",
    province_id: 16,
  },
  {
    id: 581,
    title: "سوران",
    slug: "سوران",
    province_id: 16,
  },
  {
    id: 582,
    title: "سیرکان",
    slug: "سیرکان",
    province_id: 16,
  },
  {
    id: 583,
    title: "علی اکبر",
    slug: "علی-اکبر",
    province_id: 16,
  },
  {
    id: 584,
    title: "فنوج",
    slug: "فنوج",
    province_id: 16,
  },
  {
    id: 585,
    title: "قصرقند",
    slug: "قصرقند",
    province_id: 16,
  },
  {
    id: 586,
    title: "کنارک",
    slug: "کنارک",
    province_id: 16,
  },
  {
    id: 587,
    title: "گشت",
    slug: "گشت",
    province_id: 16,
  },
  {
    id: 588,
    title: "گلمورتی",
    slug: "گلمورتی",
    province_id: 16,
  },
  {
    id: 589,
    title: "محمدان",
    slug: "محمدان",
    province_id: 16,
  },
  {
    id: 590,
    title: "محمدآباد",
    slug: "سیستان-و-بلوچستان-محمدآباد",
    province_id: 16,
  },
  {
    id: 591,
    title: "محمدی",
    slug: "محمدی",
    province_id: 16,
  },
  {
    id: 592,
    title: "میرجاوه",
    slug: "میرجاوه",
    province_id: 16,
  },
  {
    id: 593,
    title: "نصرت آباد",
    slug: "نصرت-آباد",
    province_id: 16,
  },
  {
    id: 594,
    title: "نگور",
    slug: "نگور",
    province_id: 16,
  },
  {
    id: 595,
    title: "نوک آباد",
    slug: "نوک-آباد",
    province_id: 16,
  },
  {
    id: 596,
    title: "نیک شهر",
    slug: "نیک-شهر",
    province_id: 16,
  },
  {
    id: 597,
    title: "هیدوچ",
    slug: "هیدوچ",
    province_id: 16,
  },
  {
    id: 598,
    title: "اردکان",
    slug: "فارس-اردکان",
    province_id: 17,
  },
  {
    id: 599,
    title: "ارسنجان",
    slug: "ارسنجان",
    province_id: 17,
  },
  {
    id: 600,
    title: "استهبان",
    slug: "استهبان",
    province_id: 17,
  },
  {
    id: 601,
    title: "اشکنان",
    slug: "اشکنان",
    province_id: 17,
  },
  {
    id: 602,
    title: "افزر",
    slug: "افزر",
    province_id: 17,
  },
  {
    id: 603,
    title: "اقلید",
    slug: "اقلید",
    province_id: 17,
  },
  {
    id: 604,
    title: "امام شهر",
    slug: "امام-شهر",
    province_id: 17,
  },
  {
    id: 605,
    title: "اهل",
    slug: "اهل",
    province_id: 17,
  },
  {
    id: 606,
    title: "اوز",
    slug: "اوز",
    province_id: 17,
  },
  {
    id: 607,
    title: "ایج",
    slug: "ایج",
    province_id: 17,
  },
  {
    id: 608,
    title: "ایزدخواست",
    slug: "ایزدخواست",
    province_id: 17,
  },
  {
    id: 609,
    title: "آباده",
    slug: "آباده",
    province_id: 17,
  },
  {
    id: 610,
    title: "آباده طشک",
    slug: "آباده-طشک",
    province_id: 17,
  },
  {
    id: 611,
    title: "باب انار",
    slug: "باب-انار",
    province_id: 17,
  },
  {
    id: 612,
    title: "بالاده",
    slug: "فارس-بالاده",
    province_id: 17,
  },
  {
    id: 613,
    title: "بنارویه",
    slug: "بنارویه",
    province_id: 17,
  },
  {
    id: 614,
    title: "بهمن",
    slug: "بهمن",
    province_id: 17,
  },
  {
    id: 615,
    title: "بوانات",
    slug: "بوانات",
    province_id: 17,
  },
  {
    id: 616,
    title: "بیرم",
    slug: "بیرم",
    province_id: 17,
  },
  {
    id: 617,
    title: "بیضا",
    slug: "بیضا",
    province_id: 17,
  },
  {
    id: 618,
    title: "جنت شهر",
    slug: "جنت-شهر",
    province_id: 17,
  },
  {
    id: 619,
    title: "جهرم",
    slug: "جهرم",
    province_id: 17,
  },
  {
    id: 620,
    title: "جویم",
    slug: "جویم",
    province_id: 17,
  },
  {
    id: 621,
    title: "زرین دشت",
    slug: "زرین-دشت",
    province_id: 17,
  },
  {
    id: 622,
    title: "حسن آباد",
    slug: "فارس-حسن-آباد",
    province_id: 17,
  },
  {
    id: 623,
    title: "خان زنیان",
    slug: "خان-زنیان",
    province_id: 17,
  },
  {
    id: 624,
    title: "خاوران",
    slug: "خاوران",
    province_id: 17,
  },
  {
    id: 625,
    title: "خرامه",
    slug: "خرامه",
    province_id: 17,
  },
  {
    id: 626,
    title: "خشت",
    slug: "خشت",
    province_id: 17,
  },
  {
    id: 627,
    title: "خنج",
    slug: "خنج",
    province_id: 17,
  },
  {
    id: 628,
    title: "خور",
    slug: "فارس-خور",
    province_id: 17,
  },
  {
    id: 629,
    title: "داراب",
    slug: "داراب",
    province_id: 17,
  },
  {
    id: 630,
    title: "داریان",
    slug: "داریان",
    province_id: 17,
  },
  {
    id: 631,
    title: "دبیران",
    slug: "دبیران",
    province_id: 17,
  },
  {
    id: 632,
    title: "دژکرد",
    slug: "دژکرد",
    province_id: 17,
  },
  {
    id: 633,
    title: "دهرم",
    slug: "دهرم",
    province_id: 17,
  },
  {
    id: 634,
    title: "دوبرجی",
    slug: "دوبرجی",
    province_id: 17,
  },
  {
    id: 635,
    title: "رامجرد",
    slug: "رامجرد",
    province_id: 17,
  },
  {
    id: 636,
    title: "رونیز",
    slug: "رونیز",
    province_id: 17,
  },
  {
    id: 637,
    title: "زاهدشهر",
    slug: "زاهدشهر",
    province_id: 17,
  },
  {
    id: 638,
    title: "زرقان",
    slug: "زرقان",
    province_id: 17,
  },
  {
    id: 639,
    title: "سده",
    slug: "سده",
    province_id: 17,
  },
  {
    id: 640,
    title: "سروستان",
    slug: "سروستان",
    province_id: 17,
  },
  {
    id: 641,
    title: "سعادت شهر",
    slug: "سعادت-شهر",
    province_id: 17,
  },
  {
    id: 642,
    title: "سورمق",
    slug: "سورمق",
    province_id: 17,
  },
  {
    id: 643,
    title: "سیدان",
    slug: "سیدان",
    province_id: 17,
  },
  {
    id: 644,
    title: "ششده",
    slug: "ششده",
    province_id: 17,
  },
  {
    id: 645,
    title: "شهرپیر",
    slug: "شهرپیر",
    province_id: 17,
  },
  {
    id: 646,
    title: "شهرصدرا",
    slug: "شهرصدرا",
    province_id: 17,
  },
  {
    id: 647,
    title: "شیراز",
    slug: "شیراز",
    province_id: 17,
  },
  {
    id: 648,
    title: "صغاد",
    slug: "صغاد",
    province_id: 17,
  },
  {
    id: 649,
    title: "صفاشهر",
    slug: "صفاشهر",
    province_id: 17,
  },
  {
    id: 650,
    title: "علامرودشت",
    slug: "علامرودشت",
    province_id: 17,
  },
  {
    id: 651,
    title: "فدامی",
    slug: "فدامی",
    province_id: 17,
  },
  {
    id: 652,
    title: "فراشبند",
    slug: "فراشبند",
    province_id: 17,
  },
  {
    id: 653,
    title: "فسا",
    slug: "فسا",
    province_id: 17,
  },
  {
    id: 654,
    title: "فیروزآباد",
    slug: "فارس-فیروزآباد",
    province_id: 17,
  },
  {
    id: 655,
    title: "قائمیه",
    slug: "قائمیه",
    province_id: 17,
  },
  {
    id: 656,
    title: "قادرآباد",
    slug: "قادرآباد",
    province_id: 17,
  },
  {
    id: 657,
    title: "قطب آباد",
    slug: "قطب-آباد",
    province_id: 17,
  },
  {
    id: 658,
    title: "قطرویه",
    slug: "قطرویه",
    province_id: 17,
  },
  {
    id: 659,
    title: "قیر",
    slug: "قیر",
    province_id: 17,
  },
  {
    id: 660,
    title: "کارزین (فتح آباد)",
    slug: "کارزین-فتح-آباد",
    province_id: 17,
  },
  {
    id: 661,
    title: "کازرون",
    slug: "کازرون",
    province_id: 17,
  },
  {
    id: 662,
    title: "کامفیروز",
    slug: "کامفیروز",
    province_id: 17,
  },
  {
    id: 663,
    title: "کره ای",
    slug: "کره-ای",
    province_id: 17,
  },
  {
    id: 664,
    title: "کنارتخته",
    slug: "کنارتخته",
    province_id: 17,
  },
  {
    id: 665,
    title: "کوار",
    slug: "کوار",
    province_id: 17,
  },
  {
    id: 666,
    title: "گراش",
    slug: "گراش",
    province_id: 17,
  },
  {
    id: 667,
    title: "گله دار",
    slug: "گله-دار",
    province_id: 17,
  },
  {
    id: 668,
    title: "لار",
    slug: "لار",
    province_id: 17,
  },
  {
    id: 669,
    title: "لامرد",
    slug: "لامرد",
    province_id: 17,
  },
  {
    id: 670,
    title: "لپویی",
    slug: "لپویی",
    province_id: 17,
  },
  {
    id: 671,
    title: "لطیفی",
    slug: "لطیفی",
    province_id: 17,
  },
  {
    id: 672,
    title: "مبارک آباددیز",
    slug: "مبارک-آباددیز",
    province_id: 17,
  },
  {
    id: 673,
    title: "مرودشت",
    slug: "مرودشت",
    province_id: 17,
  },
  {
    id: 674,
    title: "مشکان",
    slug: "مشکان",
    province_id: 17,
  },
  {
    id: 675,
    title: "مصیری",
    slug: "مصیری",
    province_id: 17,
  },
  {
    id: 676,
    title: "مهر",
    slug: "مهر",
    province_id: 17,
  },
  {
    id: 677,
    title: "میمند",
    slug: "میمند",
    province_id: 17,
  },
  {
    id: 678,
    title: "نوبندگان",
    slug: "نوبندگان",
    province_id: 17,
  },
  {
    id: 679,
    title: "نوجین",
    slug: "نوجین",
    province_id: 17,
  },
  {
    id: 680,
    title: "نودان",
    slug: "نودان",
    province_id: 17,
  },
  {
    id: 681,
    title: "نورآباد",
    slug: "فارس-نورآباد",
    province_id: 17,
  },
  {
    id: 682,
    title: "نی ریز",
    slug: "نی-ریز",
    province_id: 17,
  },
  {
    id: 683,
    title: "وراوی",
    slug: "وراوی",
    province_id: 17,
  },
  {
    id: 684,
    title: "ارداق",
    slug: "ارداق",
    province_id: 18,
  },
  {
    id: 685,
    title: "اسفرورین",
    slug: "اسفرورین",
    province_id: 18,
  },
  {
    id: 686,
    title: "اقبالیه",
    slug: "اقبالیه",
    province_id: 18,
  },
  {
    id: 687,
    title: "الوند",
    slug: "الوند",
    province_id: 18,
  },
  {
    id: 688,
    title: "آبگرم",
    slug: "آبگرم",
    province_id: 18,
  },
  {
    id: 689,
    title: "آبیک",
    slug: "آبیک",
    province_id: 18,
  },
  {
    id: 690,
    title: "آوج",
    slug: "آوج",
    province_id: 18,
  },
  {
    id: 691,
    title: "بوئین زهرا",
    slug: "بوئین-زهرا",
    province_id: 18,
  },
  {
    id: 692,
    title: "بیدستان",
    slug: "بیدستان",
    province_id: 18,
  },
  {
    id: 693,
    title: "تاکستان",
    slug: "تاکستان",
    province_id: 18,
  },
  {
    id: 694,
    title: "خاکعلی",
    slug: "خاکعلی",
    province_id: 18,
  },
  {
    id: 695,
    title: "خرمدشت",
    slug: "خرمدشت",
    province_id: 18,
  },
  {
    id: 696,
    title: "دانسفهان",
    slug: "دانسفهان",
    province_id: 18,
  },
  {
    id: 697,
    title: "رازمیان",
    slug: "رازمیان",
    province_id: 18,
  },
  {
    id: 698,
    title: "سگزآباد",
    slug: "سگزآباد",
    province_id: 18,
  },
  {
    id: 699,
    title: "سیردان",
    slug: "سیردان",
    province_id: 18,
  },
  {
    id: 700,
    title: "شال",
    slug: "شال",
    province_id: 18,
  },
  {
    id: 701,
    title: "شریفیه",
    slug: "شریفیه",
    province_id: 18,
  },
  {
    id: 702,
    title: "ضیاآباد",
    slug: "ضیاآباد",
    province_id: 18,
  },
  {
    id: 703,
    title: "قزوین",
    slug: "قزوین",
    province_id: 18,
  },
  {
    id: 704,
    title: "کوهین",
    slug: "کوهین",
    province_id: 18,
  },
  {
    id: 705,
    title: "محمدیه",
    slug: "محمدیه",
    province_id: 18,
  },
  {
    id: 706,
    title: "محمودآباد نمونه",
    slug: "محمودآباد-نمونه",
    province_id: 18,
  },
  {
    id: 707,
    title: "معلم کلایه",
    slug: "معلم-کلایه",
    province_id: 18,
  },
  {
    id: 708,
    title: "نرجه",
    slug: "نرجه",
    province_id: 18,
  },
  {
    id: 709,
    title: "جعفریه",
    slug: "جعفریه",
    province_id: 19,
  },
  {
    id: 710,
    title: "دستجرد",
    slug: "دستجرد",
    province_id: 19,
  },
  {
    id: 711,
    title: "سلفچگان",
    slug: "سلفچگان",
    province_id: 19,
  },
  {
    id: 712,
    title: "قم",
    slug: "قم",
    province_id: 19,
  },
  {
    id: 713,
    title: "قنوات",
    slug: "قنوات",
    province_id: 19,
  },
  {
    id: 714,
    title: "کهک",
    slug: "کهک",
    province_id: 19,
  },
  {
    id: 715,
    title: "آرمرده",
    slug: "آرمرده",
    province_id: 20,
  },
  {
    id: 716,
    title: "بابارشانی",
    slug: "بابارشانی",
    province_id: 20,
  },
  {
    id: 717,
    title: "بانه",
    slug: "بانه",
    province_id: 20,
  },
  {
    id: 718,
    title: "بلبان آباد",
    slug: "بلبان-آباد",
    province_id: 20,
  },
  {
    id: 719,
    title: "بوئین سفلی",
    slug: "بوئین-سفلی",
    province_id: 20,
  },
  {
    id: 720,
    title: "بیجار",
    slug: "بیجار",
    province_id: 20,
  },
  {
    id: 721,
    title: "چناره",
    slug: "چناره",
    province_id: 20,
  },
  {
    id: 722,
    title: "دزج",
    slug: "دزج",
    province_id: 20,
  },
  {
    id: 723,
    title: "دلبران",
    slug: "دلبران",
    province_id: 20,
  },
  {
    id: 724,
    title: "دهگلان",
    slug: "دهگلان",
    province_id: 20,
  },
  {
    id: 725,
    title: "دیواندره",
    slug: "دیواندره",
    province_id: 20,
  },
  {
    id: 726,
    title: "زرینه",
    slug: "زرینه",
    province_id: 20,
  },
  {
    id: 727,
    title: "سروآباد",
    slug: "سروآباد",
    province_id: 20,
  },
  {
    id: 728,
    title: "سریش آباد",
    slug: "سریش-آباد",
    province_id: 20,
  },
  {
    id: 729,
    title: "سقز",
    slug: "سقز",
    province_id: 20,
  },
  {
    id: 730,
    title: "سنندج",
    slug: "سنندج",
    province_id: 20,
  },
  {
    id: 731,
    title: "شویشه",
    slug: "شویشه",
    province_id: 20,
  },
  {
    id: 732,
    title: "صاحب",
    slug: "صاحب",
    province_id: 20,
  },
  {
    id: 733,
    title: "قروه",
    slug: "قروه",
    province_id: 20,
  },
  {
    id: 734,
    title: "کامیاران",
    slug: "کامیاران",
    province_id: 20,
  },
  {
    id: 735,
    title: "کانی دینار",
    slug: "کانی-دینار",
    province_id: 20,
  },
  {
    id: 736,
    title: "کانی سور",
    slug: "کانی-سور",
    province_id: 20,
  },
  {
    id: 737,
    title: "مریوان",
    slug: "مریوان",
    province_id: 20,
  },
  {
    id: 738,
    title: "موچش",
    slug: "موچش",
    province_id: 20,
  },
  {
    id: 739,
    title: "یاسوکند",
    slug: "یاسوکند",
    province_id: 20,
  },
  {
    id: 740,
    title: "اختیارآباد",
    slug: "اختیارآباد",
    province_id: 21,
  },
  {
    id: 741,
    title: "ارزوئیه",
    slug: "ارزوئیه",
    province_id: 21,
  },
  {
    id: 742,
    title: "امین شهر",
    slug: "امین-شهر",
    province_id: 21,
  },
  {
    id: 743,
    title: "انار",
    slug: "انار",
    province_id: 21,
  },
  {
    id: 744,
    title: "اندوهجرد",
    slug: "اندوهجرد",
    province_id: 21,
  },
  {
    id: 745,
    title: "باغین",
    slug: "باغین",
    province_id: 21,
  },
  {
    id: 746,
    title: "بافت",
    slug: "بافت",
    province_id: 21,
  },
  {
    id: 747,
    title: "بردسیر",
    slug: "بردسیر",
    province_id: 21,
  },
  {
    id: 748,
    title: "بروات",
    slug: "بروات",
    province_id: 21,
  },
  {
    id: 749,
    title: "بزنجان",
    slug: "بزنجان",
    province_id: 21,
  },
  {
    id: 750,
    title: "بم",
    slug: "بم",
    province_id: 21,
  },
  {
    id: 751,
    title: "بهرمان",
    slug: "بهرمان",
    province_id: 21,
  },
  {
    id: 752,
    title: "پاریز",
    slug: "پاریز",
    province_id: 21,
  },
  {
    id: 753,
    title: "جبالبارز",
    slug: "جبالبارز",
    province_id: 21,
  },
  {
    id: 754,
    title: "جوپار",
    slug: "جوپار",
    province_id: 21,
  },
  {
    id: 755,
    title: "جوزم",
    slug: "جوزم",
    province_id: 21,
  },
  {
    id: 756,
    title: "جیرفت",
    slug: "جیرفت",
    province_id: 21,
  },
  {
    id: 757,
    title: "چترود",
    slug: "چترود",
    province_id: 21,
  },
  {
    id: 758,
    title: "خاتون آباد",
    slug: "خاتون-آباد",
    province_id: 21,
  },
  {
    id: 759,
    title: "خانوک",
    slug: "خانوک",
    province_id: 21,
  },
  {
    id: 760,
    title: "خورسند",
    slug: "خورسند",
    province_id: 21,
  },
  {
    id: 761,
    title: "درب بهشت",
    slug: "درب-بهشت",
    province_id: 21,
  },
  {
    id: 762,
    title: "دهج",
    slug: "دهج",
    province_id: 21,
  },
  {
    id: 763,
    title: "رابر",
    slug: "رابر",
    province_id: 21,
  },
  {
    id: 764,
    title: "راور",
    slug: "راور",
    province_id: 21,
  },
  {
    id: 765,
    title: "راین",
    slug: "راین",
    province_id: 21,
  },
  {
    id: 766,
    title: "رفسنجان",
    slug: "رفسنجان",
    province_id: 21,
  },
  {
    id: 767,
    title: "رودبار",
    slug: "کرمان-رودبار",
    province_id: 21,
  },
  {
    id: 768,
    title: "ریحان شهر",
    slug: "ریحان-شهر",
    province_id: 21,
  },
  {
    id: 769,
    title: "زرند",
    slug: "زرند",
    province_id: 21,
  },
  {
    id: 770,
    title: "زنگی آباد",
    slug: "زنگی-آباد",
    province_id: 21,
  },
  {
    id: 771,
    title: "زیدآباد",
    slug: "زیدآباد",
    province_id: 21,
  },
  {
    id: 772,
    title: "سیرجان",
    slug: "سیرجان",
    province_id: 21,
  },
  {
    id: 773,
    title: "شهداد",
    slug: "شهداد",
    province_id: 21,
  },
  {
    id: 774,
    title: "شهربابک",
    slug: "شهربابک",
    province_id: 21,
  },
  {
    id: 775,
    title: "صفائیه",
    slug: "صفائیه",
    province_id: 21,
  },
  {
    id: 776,
    title: "عنبرآباد",
    slug: "عنبرآباد",
    province_id: 21,
  },
  {
    id: 777,
    title: "فاریاب",
    slug: "فاریاب",
    province_id: 21,
  },
  {
    id: 778,
    title: "فهرج",
    slug: "فهرج",
    province_id: 21,
  },
  {
    id: 779,
    title: "قلعه گنج",
    slug: "قلعه-گنج",
    province_id: 21,
  },
  {
    id: 780,
    title: "کاظم آباد",
    slug: "کاظم-آباد",
    province_id: 21,
  },
  {
    id: 781,
    title: "کرمان",
    slug: "کرمان",
    province_id: 21,
  },
  {
    id: 782,
    title: "کشکوئیه",
    slug: "کشکوئیه",
    province_id: 21,
  },
  {
    id: 783,
    title: "کهنوج",
    slug: "کهنوج",
    province_id: 21,
  },
  {
    id: 784,
    title: "کوهبنان",
    slug: "کوهبنان",
    province_id: 21,
  },
  {
    id: 785,
    title: "کیانشهر",
    slug: "کیانشهر",
    province_id: 21,
  },
  {
    id: 786,
    title: "گلباف",
    slug: "گلباف",
    province_id: 21,
  },
  {
    id: 787,
    title: "گلزار",
    slug: "گلزار",
    province_id: 21,
  },
  {
    id: 788,
    title: "لاله زار",
    slug: "لاله-زار",
    province_id: 21,
  },
  {
    id: 789,
    title: "ماهان",
    slug: "ماهان",
    province_id: 21,
  },
  {
    id: 790,
    title: "محمدآباد",
    slug: "کرمان-محمدآباد",
    province_id: 21,
  },
  {
    id: 791,
    title: "محی آباد",
    slug: "محی-آباد",
    province_id: 21,
  },
  {
    id: 792,
    title: "مردهک",
    slug: "مردهک",
    province_id: 21,
  },
  {
    id: 793,
    title: "مس سرچشمه",
    slug: "مس-سرچشمه",
    province_id: 21,
  },
  {
    id: 794,
    title: "منوجان",
    slug: "منوجان",
    province_id: 21,
  },
  {
    id: 795,
    title: "نجف شهر",
    slug: "نجف-شهر",
    province_id: 21,
  },
  {
    id: 796,
    title: "نرماشیر",
    slug: "نرماشیر",
    province_id: 21,
  },
  {
    id: 797,
    title: "نظام شهر",
    slug: "نظام-شهر",
    province_id: 21,
  },
  {
    id: 798,
    title: "نگار",
    slug: "نگار",
    province_id: 21,
  },
  {
    id: 799,
    title: "نودژ",
    slug: "نودژ",
    province_id: 21,
  },
  {
    id: 800,
    title: "هجدک",
    slug: "هجدک",
    province_id: 21,
  },
  {
    id: 801,
    title: "یزدان شهر",
    slug: "یزدان-شهر",
    province_id: 21,
  },
  {
    id: 802,
    title: "ازگله",
    slug: "ازگله",
    province_id: 22,
  },
  {
    id: 803,
    title: "اسلام آباد غرب",
    slug: "اسلام-آباد-غرب",
    province_id: 22,
  },
  {
    id: 804,
    title: "باینگان",
    slug: "باینگان",
    province_id: 22,
  },
  {
    id: 805,
    title: "بیستون",
    slug: "بیستون",
    province_id: 22,
  },
  {
    id: 806,
    title: "پاوه",
    slug: "پاوه",
    province_id: 22,
  },
  {
    id: 807,
    title: "تازه آباد",
    slug: "تازه-آباد",
    province_id: 22,
  },
  {
    id: 808,
    title: "جوان رود",
    slug: "جوان-رود",
    province_id: 22,
  },
  {
    id: 809,
    title: "حمیل",
    slug: "حمیل",
    province_id: 22,
  },
  {
    id: 810,
    title: "ماهیدشت",
    slug: "ماهیدشت",
    province_id: 22,
  },
  {
    id: 811,
    title: "روانسر",
    slug: "روانسر",
    province_id: 22,
  },
  {
    id: 812,
    title: "سرپل ذهاب",
    slug: "سرپل-ذهاب",
    province_id: 22,
  },
  {
    id: 813,
    title: "سرمست",
    slug: "سرمست",
    province_id: 22,
  },
  {
    id: 814,
    title: "سطر",
    slug: "سطر",
    province_id: 22,
  },
  {
    id: 815,
    title: "سنقر",
    slug: "سنقر",
    province_id: 22,
  },
  {
    id: 816,
    title: "سومار",
    slug: "سومار",
    province_id: 22,
  },
  {
    id: 817,
    title: "شاهو",
    slug: "شاهو",
    province_id: 22,
  },
  {
    id: 818,
    title: "صحنه",
    slug: "صحنه",
    province_id: 22,
  },
  {
    id: 819,
    title: "قصرشیرین",
    slug: "قصرشیرین",
    province_id: 22,
  },
  {
    id: 820,
    title: "کرمانشاه",
    slug: "کرمانشاه",
    province_id: 22,
  },
  {
    id: 821,
    title: "کرندغرب",
    slug: "کرندغرب",
    province_id: 22,
  },
  {
    id: 822,
    title: "کنگاور",
    slug: "کنگاور",
    province_id: 22,
  },
  {
    id: 823,
    title: "کوزران",
    slug: "کوزران",
    province_id: 22,
  },
  {
    id: 824,
    title: "گهواره",
    slug: "گهواره",
    province_id: 22,
  },
  {
    id: 825,
    title: "گیلانغرب",
    slug: "گیلانغرب",
    province_id: 22,
  },
  {
    id: 826,
    title: "میان راهان",
    slug: "میان-راهان",
    province_id: 22,
  },
  {
    id: 827,
    title: "نودشه",
    slug: "نودشه",
    province_id: 22,
  },
  {
    id: 828,
    title: "نوسود",
    slug: "نوسود",
    province_id: 22,
  },
  {
    id: 829,
    title: "هرسین",
    slug: "هرسین",
    province_id: 22,
  },
  {
    id: 830,
    title: "هلشی",
    slug: "هلشی",
    province_id: 22,
  },
  {
    id: 831,
    title: "باشت",
    slug: "باشت",
    province_id: 23,
  },
  {
    id: 832,
    title: "پاتاوه",
    slug: "پاتاوه",
    province_id: 23,
  },
  {
    id: 833,
    title: "چرام",
    slug: "چرام",
    province_id: 23,
  },
  {
    id: 834,
    title: "چیتاب",
    slug: "چیتاب",
    province_id: 23,
  },
  {
    id: 835,
    title: "دهدشت",
    slug: "دهدشت",
    province_id: 23,
  },
  {
    id: 836,
    title: "دوگنبدان",
    slug: "دوگنبدان",
    province_id: 23,
  },
  {
    id: 837,
    title: "دیشموک",
    slug: "دیشموک",
    province_id: 23,
  },
  {
    id: 838,
    title: "سوق",
    slug: "سوق",
    province_id: 23,
  },
  {
    id: 839,
    title: "سی سخت",
    slug: "سی-سخت",
    province_id: 23,
  },
  {
    id: 840,
    title: "قلعه رئیسی",
    slug: "قلعه-رئیسی",
    province_id: 23,
  },
  {
    id: 841,
    title: "گراب سفلی",
    slug: "گراب-سفلی",
    province_id: 23,
  },
  {
    id: 842,
    title: "لنده",
    slug: "لنده",
    province_id: 23,
  },
  {
    id: 843,
    title: "لیکک",
    slug: "لیکک",
    province_id: 23,
  },
  {
    id: 844,
    title: "مادوان",
    slug: "مادوان",
    province_id: 23,
  },
  {
    id: 845,
    title: "مارگون",
    slug: "مارگون",
    province_id: 23,
  },
  {
    id: 846,
    title: "یاسوج",
    slug: "یاسوج",
    province_id: 23,
  },
  {
    id: 847,
    title: "انبارآلوم",
    slug: "انبارآلوم",
    province_id: 24,
  },
  {
    id: 848,
    title: "اینچه برون",
    slug: "اینچه-برون",
    province_id: 24,
  },
  {
    id: 849,
    title: "آزادشهر",
    slug: "آزادشهر",
    province_id: 24,
  },
  {
    id: 850,
    title: "آق قلا",
    slug: "آق-قلا",
    province_id: 24,
  },
  {
    id: 851,
    title: "بندرترکمن",
    slug: "بندرترکمن",
    province_id: 24,
  },
  {
    id: 852,
    title: "بندرگز",
    slug: "بندرگز",
    province_id: 24,
  },
  {
    id: 853,
    title: "جلین",
    slug: "جلین",
    province_id: 24,
  },
  {
    id: 854,
    title: "خان ببین",
    slug: "خان-ببین",
    province_id: 24,
  },
  {
    id: 855,
    title: "دلند",
    slug: "دلند",
    province_id: 24,
  },
  {
    id: 856,
    title: "رامیان",
    slug: "رامیان",
    province_id: 24,
  },
  {
    id: 857,
    title: "سرخنکلاته",
    slug: "سرخنکلاته",
    province_id: 24,
  },
  {
    id: 858,
    title: "سیمین شهر",
    slug: "سیمین-شهر",
    province_id: 24,
  },
  {
    id: 859,
    title: "علی آباد کتول",
    slug: "علی-آباد-کتول",
    province_id: 24,
  },
  {
    id: 860,
    title: "فاضل آباد",
    slug: "فاضل-آباد",
    province_id: 24,
  },
  {
    id: 861,
    title: "کردکوی",
    slug: "کردکوی",
    province_id: 24,
  },
  {
    id: 862,
    title: "کلاله",
    slug: "کلاله",
    province_id: 24,
  },
  {
    id: 863,
    title: "گالیکش",
    slug: "گالیکش",
    province_id: 24,
  },
  {
    id: 864,
    title: "گرگان",
    slug: "گرگان",
    province_id: 24,
  },
  {
    id: 865,
    title: "گمیش تپه",
    slug: "گمیش-تپه",
    province_id: 24,
  },
  {
    id: 866,
    title: "گنبدکاووس",
    slug: "گنبدکاووس",
    province_id: 24,
  },
  {
    id: 867,
    title: "مراوه",
    slug: "مراوه",
    province_id: 24,
  },
  {
    id: 868,
    title: "مینودشت",
    slug: "مینودشت",
    province_id: 24,
  },
  {
    id: 869,
    title: "نگین شهر",
    slug: "نگین-شهر",
    province_id: 24,
  },
  {
    id: 870,
    title: "نوده خاندوز",
    slug: "نوده-خاندوز",
    province_id: 24,
  },
  {
    id: 871,
    title: "نوکنده",
    slug: "نوکنده",
    province_id: 24,
  },
  {
    id: 872,
    title: "ازنا",
    slug: "ازنا",
    province_id: 25,
  },
  {
    id: 873,
    title: "اشترینان",
    slug: "اشترینان",
    province_id: 25,
  },
  {
    id: 874,
    title: "الشتر",
    slug: "الشتر",
    province_id: 25,
  },
  {
    id: 875,
    title: "الیگودرز",
    slug: "الیگودرز",
    province_id: 25,
  },
  {
    id: 876,
    title: "بروجرد",
    slug: "بروجرد",
    province_id: 25,
  },
  {
    id: 877,
    title: "پلدختر",
    slug: "پلدختر",
    province_id: 25,
  },
  {
    id: 878,
    title: "چالانچولان",
    slug: "چالانچولان",
    province_id: 25,
  },
  {
    id: 879,
    title: "چغلوندی",
    slug: "چغلوندی",
    province_id: 25,
  },
  {
    id: 880,
    title: "چقابل",
    slug: "چقابل",
    province_id: 25,
  },
  {
    id: 881,
    title: "خرم آباد",
    slug: "لرستان-خرم-آباد",
    province_id: 25,
  },
  {
    id: 882,
    title: "درب گنبد",
    slug: "درب-گنبد",
    province_id: 25,
  },
  {
    id: 883,
    title: "دورود",
    slug: "دورود",
    province_id: 25,
  },
  {
    id: 884,
    title: "زاغه",
    slug: "زاغه",
    province_id: 25,
  },
  {
    id: 885,
    title: "سپیددشت",
    slug: "سپیددشت",
    province_id: 25,
  },
  {
    id: 886,
    title: "سراب دوره",
    slug: "سراب-دوره",
    province_id: 25,
  },
  {
    id: 887,
    title: "فیروزآباد",
    slug: "لرستان-فیروزآباد",
    province_id: 25,
  },
  {
    id: 888,
    title: "کونانی",
    slug: "کونانی",
    province_id: 25,
  },
  {
    id: 889,
    title: "کوهدشت",
    slug: "کوهدشت",
    province_id: 25,
  },
  {
    id: 890,
    title: "گراب",
    slug: "گراب",
    province_id: 25,
  },
  {
    id: 891,
    title: "معمولان",
    slug: "معمولان",
    province_id: 25,
  },
  {
    id: 892,
    title: "مومن آباد",
    slug: "مومن-آباد",
    province_id: 25,
  },
  {
    id: 893,
    title: "نورآباد",
    slug: "لرستان-نورآباد",
    province_id: 25,
  },
  {
    id: 894,
    title: "ویسیان",
    slug: "ویسیان",
    province_id: 25,
  },
  {
    id: 895,
    title: "احمدسرگوراب",
    slug: "احمدسرگوراب",
    province_id: 26,
  },
  {
    id: 896,
    title: "اسالم",
    slug: "اسالم",
    province_id: 26,
  },
  {
    id: 897,
    title: "اطاقور",
    slug: "اطاقور",
    province_id: 26,
  },
  {
    id: 898,
    title: "املش",
    slug: "املش",
    province_id: 26,
  },
  {
    id: 899,
    title: "آستارا",
    slug: "آستارا",
    province_id: 26,
  },
  {
    id: 900,
    title: "آستانه اشرفیه",
    slug: "آستانه-اشرفیه",
    province_id: 26,
  },
  {
    id: 901,
    title: "بازار جمعه",
    slug: "بازار-جمعه",
    province_id: 26,
  },
  {
    id: 902,
    title: "بره سر",
    slug: "بره-سر",
    province_id: 26,
  },
  {
    id: 903,
    title: "بندرانزلی",
    slug: "بندرانزلی",
    province_id: 26,
  },
  {
    id: 906,
    title: "پره سر",
    slug: "پره-سر",
    province_id: 26,
  },
  {
    id: 907,
    title: "تالش",
    slug: "تالش",
    province_id: 26,
  },
  {
    id: 908,
    title: "توتکابن",
    slug: "توتکابن",
    province_id: 26,
  },
  {
    id: 909,
    title: "جیرنده",
    slug: "جیرنده",
    province_id: 26,
  },
  {
    id: 910,
    title: "چابکسر",
    slug: "چابکسر",
    province_id: 26,
  },
  {
    id: 911,
    title: "چاف و چمخاله",
    slug: "چاف-و-چمخاله",
    province_id: 26,
  },
  {
    id: 912,
    title: "چوبر",
    slug: "چوبر",
    province_id: 26,
  },
  {
    id: 913,
    title: "حویق",
    slug: "حویق",
    province_id: 26,
  },
  {
    id: 914,
    title: "خشکبیجار",
    slug: "خشکبیجار",
    province_id: 26,
  },
  {
    id: 915,
    title: "خمام",
    slug: "خمام",
    province_id: 26,
  },
  {
    id: 916,
    title: "دیلمان",
    slug: "دیلمان",
    province_id: 26,
  },
  {
    id: 917,
    title: "رانکوه",
    slug: "رانکوه",
    province_id: 26,
  },
  {
    id: 918,
    title: "رحیم آباد",
    slug: "رحیم-آباد",
    province_id: 26,
  },
  {
    id: 919,
    title: "رستم آباد",
    slug: "رستم-آباد",
    province_id: 26,
  },
  {
    id: 920,
    title: "رشت",
    slug: "رشت",
    province_id: 26,
  },
  {
    id: 921,
    title: "رضوانشهر",
    slug: "گیلان-رضوانشهر",
    province_id: 26,
  },
  {
    id: 922,
    title: "رودبار",
    slug: "گیلان-رودبار",
    province_id: 26,
  },
  {
    id: 923,
    title: "رودبنه",
    slug: "رودبنه",
    province_id: 26,
  },
  {
    id: 924,
    title: "رودسر",
    slug: "رودسر",
    province_id: 26,
  },
  {
    id: 925,
    title: "سنگر",
    slug: "سنگر",
    province_id: 26,
  },
  {
    id: 926,
    title: "سیاهکل",
    slug: "سیاهکل",
    province_id: 26,
  },
  {
    id: 927,
    title: "شفت",
    slug: "شفت",
    province_id: 26,
  },
  {
    id: 928,
    title: "شلمان",
    slug: "شلمان",
    province_id: 26,
  },
  {
    id: 929,
    title: "صومعه سرا",
    slug: "صومعه-سرا",
    province_id: 26,
  },
  {
    id: 930,
    title: "فومن",
    slug: "فومن",
    province_id: 26,
  },
  {
    id: 931,
    title: "کلاچای",
    slug: "کلاچای",
    province_id: 26,
  },
  {
    id: 932,
    title: "کوچصفهان",
    slug: "کوچصفهان",
    province_id: 26,
  },
  {
    id: 933,
    title: "کومله",
    slug: "کومله",
    province_id: 26,
  },
  {
    id: 934,
    title: "کیاشهر",
    slug: "کیاشهر",
    province_id: 26,
  },
  {
    id: 935,
    title: "گوراب زرمیخ",
    slug: "گوراب-زرمیخ",
    province_id: 26,
  },
  {
    id: 936,
    title: "لاهیجان",
    slug: "لاهیجان",
    province_id: 26,
  },
  {
    id: 937,
    title: "لشت نشا",
    slug: "لشت-نشا",
    province_id: 26,
  },
  {
    id: 938,
    title: "لنگرود",
    slug: "لنگرود",
    province_id: 26,
  },
  {
    id: 939,
    title: "لوشان",
    slug: "لوشان",
    province_id: 26,
  },
  {
    id: 940,
    title: "لولمان",
    slug: "لولمان",
    province_id: 26,
  },
  {
    id: 941,
    title: "لوندویل",
    slug: "لوندویل",
    province_id: 26,
  },
  {
    id: 942,
    title: "لیسار",
    slug: "لیسار",
    province_id: 26,
  },
  {
    id: 943,
    title: "ماسال",
    slug: "ماسال",
    province_id: 26,
  },
  {
    id: 944,
    title: "ماسوله",
    slug: "ماسوله",
    province_id: 26,
  },
  {
    id: 945,
    title: "مرجقل",
    slug: "مرجقل",
    province_id: 26,
  },
  {
    id: 946,
    title: "منجیل",
    slug: "منجیل",
    province_id: 26,
  },
  {
    id: 947,
    title: "واجارگاه",
    slug: "واجارگاه",
    province_id: 26,
  },
  {
    id: 948,
    title: "امیرکلا",
    slug: "امیرکلا",
    province_id: 27,
  },
  {
    id: 949,
    title: "ایزدشهر",
    slug: "ایزدشهر",
    province_id: 27,
  },
  {
    id: 950,
    title: "آلاشت",
    slug: "آلاشت",
    province_id: 27,
  },
  {
    id: 951,
    title: "آمل",
    slug: "آمل",
    province_id: 27,
  },
  {
    id: 952,
    title: "بابل",
    slug: "بابل",
    province_id: 27,
  },
  {
    id: 953,
    title: "بابلسر",
    slug: "بابلسر",
    province_id: 27,
  },
  {
    id: 954,
    title: "بلده",
    slug: "مازندران-بلده",
    province_id: 27,
  },
  {
    id: 955,
    title: "بهشهر",
    slug: "بهشهر",
    province_id: 27,
  },
  {
    id: 956,
    title: "بهنمیر",
    slug: "بهنمیر",
    province_id: 27,
  },
  {
    id: 957,
    title: "پل سفید",
    slug: "پل-سفید",
    province_id: 27,
  },
  {
    id: 958,
    title: "تنکابن",
    slug: "تنکابن",
    province_id: 27,
  },
  {
    id: 959,
    title: "جویبار",
    slug: "جویبار",
    province_id: 27,
  },
  {
    id: 960,
    title: "چالوس",
    slug: "چالوس",
    province_id: 27,
  },
  {
    id: 961,
    title: "چمستان",
    slug: "چمستان",
    province_id: 27,
  },
  {
    id: 962,
    title: "خرم آباد",
    slug: "مازندران-خرم-آباد",
    province_id: 27,
  },
  {
    id: 963,
    title: "خلیل شهر",
    slug: "خلیل-شهر",
    province_id: 27,
  },
  {
    id: 964,
    title: "خوش رودپی",
    slug: "خوش-رودپی",
    province_id: 27,
  },
  {
    id: 965,
    title: "دابودشت",
    slug: "دابودشت",
    province_id: 27,
  },
  {
    id: 966,
    title: "رامسر",
    slug: "رامسر",
    province_id: 27,
  },
  {
    id: 967,
    title: "رستمکلا",
    slug: "رستمکلا",
    province_id: 27,
  },
  {
    id: 968,
    title: "رویان",
    slug: "رویان",
    province_id: 27,
  },
  {
    id: 969,
    title: "رینه",
    slug: "رینه",
    province_id: 27,
  },
  {
    id: 970,
    title: "زرگرمحله",
    slug: "زرگرمحله",
    province_id: 27,
  },
  {
    id: 971,
    title: "زیرآب",
    slug: "زیرآب",
    province_id: 27,
  },
  {
    id: 972,
    title: "ساری",
    slug: "ساری",
    province_id: 27,
  },
  {
    id: 973,
    title: "سرخرود",
    slug: "سرخرود",
    province_id: 27,
  },
  {
    id: 974,
    title: "سلمان شهر",
    slug: "سلمان-شهر",
    province_id: 27,
  },
  {
    id: 975,
    title: "سورک",
    slug: "سورک",
    province_id: 27,
  },
  {
    id: 976,
    title: "شیرگاه",
    slug: "شیرگاه",
    province_id: 27,
  },
  {
    id: 977,
    title: "شیرود",
    slug: "شیرود",
    province_id: 27,
  },
  {
    id: 978,
    title: "عباس آباد",
    slug: "عباس-آباد",
    province_id: 27,
  },
  {
    id: 979,
    title: "فریدونکنار",
    slug: "فریدونکنار",
    province_id: 27,
  },
  {
    id: 980,
    title: "فریم",
    slug: "فریم",
    province_id: 27,
  },
  {
    id: 981,
    title: "قائم شهر",
    slug: "قائم-شهر",
    province_id: 27,
  },
  {
    id: 982,
    title: "کتالم",
    slug: "کتالم",
    province_id: 27,
  },
  {
    id: 983,
    title: "کلارآباد",
    slug: "کلارآباد",
    province_id: 27,
  },
  {
    id: 984,
    title: "کلاردشت",
    slug: "کلاردشت",
    province_id: 27,
  },
  {
    id: 985,
    title: "کله بست",
    slug: "کله-بست",
    province_id: 27,
  },
  {
    id: 986,
    title: "کوهی خیل",
    slug: "کوهی-خیل",
    province_id: 27,
  },
  {
    id: 987,
    title: "کیاسر",
    slug: "کیاسر",
    province_id: 27,
  },
  {
    id: 988,
    title: "کیاکلا",
    slug: "کیاکلا",
    province_id: 27,
  },
  {
    id: 989,
    title: "گتاب",
    slug: "گتاب",
    province_id: 27,
  },
  {
    id: 990,
    title: "گزنک",
    slug: "گزنک",
    province_id: 27,
  },
  {
    id: 991,
    title: "گلوگاه",
    slug: "گلوگاه",
    province_id: 27,
  },
  {
    id: 992,
    title: "محمودآباد",
    slug: "مازندران-محمودآباد",
    province_id: 27,
  },
  {
    id: 993,
    title: "مرزن آباد",
    slug: "مرزن-آباد",
    province_id: 27,
  },
  {
    id: 994,
    title: "مرزیکلا",
    slug: "مرزیکلا",
    province_id: 27,
  },
  {
    id: 995,
    title: "نشتارود",
    slug: "نشتارود",
    province_id: 27,
  },
  {
    id: 996,
    title: "نکا",
    slug: "نکا",
    province_id: 27,
  },
  {
    id: 997,
    title: "نور",
    slug: "نور",
    province_id: 27,
  },
  {
    id: 998,
    title: "نوشهر",
    slug: "نوشهر",
    province_id: 27,
  },
  {
    id: 999,
    title: "اراک",
    slug: "اراک",
    province_id: 28,
  },
  {
    id: 1000,
    title: "آستانه",
    slug: "آستانه",
    province_id: 28,
  },
  {
    id: 1001,
    title: "آشتیان",
    slug: "آشتیان",
    province_id: 28,
  },
  {
    id: 1002,
    title: "پرندک",
    slug: "پرندک",
    province_id: 28,
  },
  {
    id: 1003,
    title: "تفرش",
    slug: "تفرش",
    province_id: 28,
  },
  {
    id: 1004,
    title: "توره",
    slug: "توره",
    province_id: 28,
  },
  {
    id: 1005,
    title: "جاورسیان",
    slug: "جاورسیان",
    province_id: 28,
  },
  {
    id: 1006,
    title: "خشکرود",
    slug: "خشکرود",
    province_id: 28,
  },
  {
    id: 1007,
    title: "خمین",
    slug: "خمین",
    province_id: 28,
  },
  {
    id: 1008,
    title: "خنداب",
    slug: "خنداب",
    province_id: 28,
  },
  {
    id: 1009,
    title: "داودآباد",
    slug: "داودآباد",
    province_id: 28,
  },
  {
    id: 1010,
    title: "دلیجان",
    slug: "دلیجان",
    province_id: 28,
  },
  {
    id: 1011,
    title: "رازقان",
    slug: "رازقان",
    province_id: 28,
  },
  {
    id: 1012,
    title: "زاویه",
    slug: "زاویه",
    province_id: 28,
  },
  {
    id: 1013,
    title: "ساروق",
    slug: "ساروق",
    province_id: 28,
  },
  {
    id: 1014,
    title: "ساوه",
    slug: "ساوه",
    province_id: 28,
  },
  {
    id: 1015,
    title: "سنجان",
    slug: "سنجان",
    province_id: 28,
  },
  {
    id: 1016,
    title: "شازند",
    slug: "شازند",
    province_id: 28,
  },
  {
    id: 1017,
    title: "غرق آباد",
    slug: "غرق-آباد",
    province_id: 28,
  },
  {
    id: 1018,
    title: "فرمهین",
    slug: "فرمهین",
    province_id: 28,
  },
  {
    id: 1019,
    title: "قورچی باشی",
    slug: "قورچی-باشی",
    province_id: 28,
  },
  {
    id: 1020,
    title: "کرهرود",
    slug: "کرهرود",
    province_id: 28,
  },
  {
    id: 1021,
    title: "کمیجان",
    slug: "کمیجان",
    province_id: 28,
  },
  {
    id: 1022,
    title: "مامونیه",
    slug: "مامونیه",
    province_id: 28,
  },
  {
    id: 1023,
    title: "محلات",
    slug: "محلات",
    province_id: 28,
  },
  {
    id: 1024,
    title: "مهاجران",
    slug: "مهاجران",
    province_id: 28,
  },
  {
    id: 1025,
    title: "میلاجرد",
    slug: "میلاجرد",
    province_id: 28,
  },
  {
    id: 1026,
    title: "نراق",
    slug: "نراق",
    province_id: 28,
  },
  {
    id: 1027,
    title: "نوبران",
    slug: "نوبران",
    province_id: 28,
  },
  {
    id: 1028,
    title: "نیمور",
    slug: "نیمور",
    province_id: 28,
  },
  {
    id: 1029,
    title: "هندودر",
    slug: "هندودر",
    province_id: 28,
  },
  {
    id: 1030,
    title: "ابوموسی",
    slug: "ابوموسی",
    province_id: 29,
  },
  {
    id: 1031,
    title: "بستک",
    slug: "بستک",
    province_id: 29,
  },
  {
    id: 1032,
    title: "بندرجاسک",
    slug: "بندرجاسک",
    province_id: 29,
  },
  {
    id: 1033,
    title: "بندرچارک",
    slug: "بندرچارک",
    province_id: 29,
  },
  {
    id: 1034,
    title: "بندرخمیر",
    slug: "بندرخمیر",
    province_id: 29,
  },
  {
    id: 1035,
    title: "بندرعباس",
    slug: "بندرعباس",
    province_id: 29,
  },
  {
    id: 1036,
    title: "بندرلنگه",
    slug: "بندرلنگه",
    province_id: 29,
  },
  {
    id: 1037,
    title: "بیکا",
    slug: "بیکا",
    province_id: 29,
  },
  {
    id: 1038,
    title: "پارسیان",
    slug: "پارسیان",
    province_id: 29,
  },
  {
    id: 1039,
    title: "تخت",
    slug: "تخت",
    province_id: 29,
  },
  {
    id: 1040,
    title: "جناح",
    slug: "جناح",
    province_id: 29,
  },
  {
    id: 1041,
    title: "حاجی آباد",
    slug: "هرمزگان-حاجی-آباد",
    province_id: 29,
  },
  {
    id: 1042,
    title: "درگهان",
    slug: "درگهان",
    province_id: 29,
  },
  {
    id: 1043,
    title: "دهبارز",
    slug: "دهبارز",
    province_id: 29,
  },
  {
    id: 1044,
    title: "رویدر",
    slug: "رویدر",
    province_id: 29,
  },
  {
    id: 1045,
    title: "زیارتعلی",
    slug: "زیارتعلی",
    province_id: 29,
  },
  {
    id: 1046,
    title: "سردشت",
    slug: "هرمزگان-سردشت",
    province_id: 29,
  },
  {
    id: 1047,
    title: "سندرک",
    slug: "سندرک",
    province_id: 29,
  },
  {
    id: 1048,
    title: "سوزا",
    slug: "سوزا",
    province_id: 29,
  },
  {
    id: 1049,
    title: "سیریک",
    slug: "سیریک",
    province_id: 29,
  },
  {
    id: 1050,
    title: "فارغان",
    slug: "فارغان",
    province_id: 29,
  },
  {
    id: 1051,
    title: "فین",
    slug: "فین",
    province_id: 29,
  },
  {
    id: 1052,
    title: "قشم",
    slug: "قشم",
    province_id: 29,
  },
  {
    id: 1053,
    title: "قلعه قاضی",
    slug: "قلعه-قاضی",
    province_id: 29,
  },
  {
    id: 1054,
    title: "کنگ",
    slug: "کنگ",
    province_id: 29,
  },
  {
    id: 1055,
    title: "کوشکنار",
    slug: "کوشکنار",
    province_id: 29,
  },
  {
    id: 1056,
    title: "کیش",
    slug: "کیش",
    province_id: 29,
  },
  {
    id: 1057,
    title: "گوهران",
    slug: "گوهران",
    province_id: 29,
  },
  {
    id: 1058,
    title: "میناب",
    slug: "میناب",
    province_id: 29,
  },
  {
    id: 1059,
    title: "هرمز",
    slug: "هرمز",
    province_id: 29,
  },
  {
    id: 1060,
    title: "هشتبندی",
    slug: "هشتبندی",
    province_id: 29,
  },
  {
    id: 1061,
    title: "ازندریان",
    slug: "ازندریان",
    province_id: 30,
  },
  {
    id: 1062,
    title: "اسدآباد",
    slug: "اسدآباد",
    province_id: 30,
  },
  {
    id: 1063,
    title: "برزول",
    slug: "برزول",
    province_id: 30,
  },
  {
    id: 1064,
    title: "بهار",
    slug: "بهار",
    province_id: 30,
  },
  {
    id: 1065,
    title: "تویسرکان",
    slug: "تویسرکان",
    province_id: 30,
  },
  {
    id: 1066,
    title: "جورقان",
    slug: "جورقان",
    province_id: 30,
  },
  {
    id: 1067,
    title: "جوکار",
    slug: "جوکار",
    province_id: 30,
  },
  {
    id: 1068,
    title: "دمق",
    slug: "دمق",
    province_id: 30,
  },
  {
    id: 1069,
    title: "رزن",
    slug: "رزن",
    province_id: 30,
  },
  {
    id: 1070,
    title: "زنگنه",
    slug: "زنگنه",
    province_id: 30,
  },
  {
    id: 1071,
    title: "سامن",
    slug: "سامن",
    province_id: 30,
  },
  {
    id: 1072,
    title: "سرکان",
    slug: "سرکان",
    province_id: 30,
  },
  {
    id: 1073,
    title: "شیرین سو",
    slug: "شیرین-سو",
    province_id: 30,
  },
  {
    id: 1074,
    title: "صالح آباد",
    slug: "همدان-صالح-آباد",
    province_id: 30,
  },
  {
    id: 1075,
    title: "فامنین",
    slug: "فامنین",
    province_id: 30,
  },
  {
    id: 1076,
    title: "فرسفج",
    slug: "فرسفج",
    province_id: 30,
  },
  {
    id: 1077,
    title: "فیروزان",
    slug: "فیروزان",
    province_id: 30,
  },
  {
    id: 1078,
    title: "قروه درجزین",
    slug: "قروه-درجزین",
    province_id: 30,
  },
  {
    id: 1079,
    title: "قهاوند",
    slug: "قهاوند",
    province_id: 30,
  },
  {
    id: 1080,
    title: "کبودر آهنگ",
    slug: "کبودر-آهنگ",
    province_id: 30,
  },
  {
    id: 1081,
    title: "گل تپه",
    slug: "گل-تپه",
    province_id: 30,
  },
  {
    id: 1082,
    title: "گیان",
    slug: "گیان",
    province_id: 30,
  },
  {
    id: 1083,
    title: "لالجین",
    slug: "لالجین",
    province_id: 30,
  },
  {
    id: 1084,
    title: "مریانج",
    slug: "مریانج",
    province_id: 30,
  },
  {
    id: 1085,
    title: "ملایر",
    slug: "ملایر",
    province_id: 30,
  },
  {
    id: 1086,
    title: "نهاوند",
    slug: "نهاوند",
    province_id: 30,
  },
  {
    id: 1087,
    title: "همدان",
    slug: "همدان",
    province_id: 30,
  },
  {
    id: 1088,
    title: "ابرکوه",
    slug: "ابرکوه",
    province_id: 31,
  },
  {
    id: 1089,
    title: "احمدآباد",
    slug: "احمدآباد",
    province_id: 31,
  },
  {
    id: 1090,
    title: "اردکان",
    slug: "یزد-اردکان",
    province_id: 31,
  },
  {
    id: 1091,
    title: "اشکذر",
    slug: "اشکذر",
    province_id: 31,
  },
  {
    id: 1092,
    title: "بافق",
    slug: "بافق",
    province_id: 31,
  },
  {
    id: 1093,
    title: "بفروئیه",
    slug: "بفروئیه",
    province_id: 31,
  },
  {
    id: 1094,
    title: "بهاباد",
    slug: "بهاباد",
    province_id: 31,
  },
  {
    id: 1095,
    title: "تفت",
    slug: "تفت",
    province_id: 31,
  },
  {
    id: 1096,
    title: "حمیدیا",
    slug: "حمیدیا",
    province_id: 31,
  },
  {
    id: 1097,
    title: "خضرآباد",
    slug: "خضرآباد",
    province_id: 31,
  },
  {
    id: 1098,
    title: "دیهوک",
    slug: "دیهوک",
    province_id: 31,
  },
  {
    id: 1099,
    title: "زارچ",
    slug: "زارچ",
    province_id: 31,
  },
  {
    id: 1100,
    title: "شاهدیه",
    slug: "شاهدیه",
    province_id: 31,
  },
  {
    id: 1101,
    title: "طبس",
    slug: "یزد-طبس",
    province_id: 31,
  },
  {
    id: 1103,
    title: "عقدا",
    slug: "عقدا",
    province_id: 31,
  },
  {
    id: 1104,
    title: "مروست",
    slug: "مروست",
    province_id: 31,
  },
  {
    id: 1105,
    title: "مهردشت",
    slug: "مهردشت",
    province_id: 31,
  },
  {
    id: 1106,
    title: "مهریز",
    slug: "مهریز",
    province_id: 31,
  },
  {
    id: 1107,
    title: "میبد",
    slug: "میبد",
    province_id: 31,
  },
  {
    id: 1108,
    title: "ندوشن",
    slug: "ندوشن",
    province_id: 31,
  },
  {
    id: 1109,
    title: "نیر",
    slug: "یزد-نیر",
    province_id: 31,
  },
  {
    id: 1110,
    title: "هرات",
    slug: "هرات",
    province_id: 31,
  },
  {
    id: 1111,
    title: "یزد",
    slug: "یزد",
    province_id: 31,
  },
  {
    id: 1116,
    title: "پرند",
    slug: "پرند",
    province_id: 8,
  },
  {
    id: 1117,
    title: "فردیس",
    slug: "فردیس",
    province_id: 5,
  },
  {
    id: 1118,
    title: "مارلیک",
    slug: "مارلیک",
    province_id: 5,
  },
  {
    id: 1119,
    title: "سادات شهر",
    slug: "سادات-شهر",
    province_id: 27,
  },
  {
    id: 1121,
    title: "زیباکنار",
    slug: "زیباکنار",
    province_id: 26,
  },
  {
    id: 1135,
    title: "کردان",
    slug: "کردان",
    province_id: 5,
  },
  {
    id: 1137,
    title: "ساوجبلاغ",
    slug: "ساوجبلاغ",
    province_id: 5,
  },
  {
    id: 1138,
    title: "تهران دشت",
    slug: "تهران-دشت",
    province_id: 5,
  },
  {
    id: 1150,
    title: "گلبهار",
    slug: "گلبهار",
    province_id: 11,
  },
  {
    id: 1153,
    title: "قیامدشت",
    slug: "قیامدشت",
    province_id: 8,
  },
  {
    id: 1155,
    title: "بینالود",
    slug: "بینالود",
    province_id: 11,
  },
  {
    id: 1159,
    title: "پیربازار",
    slug: "پیربازار",
    province_id: 26,
  },
  {
    id: 1160,
    title: "رضوانشهر",
    slug: "رضوانشهر",
    province_id: 31,
  },
];

export const provinces = [
  {
    id: 1,
    title: "آذربایجان شرقی",
    slug: "آذربایجان-شرقی",
    tel_prefix: "041",
  },
  {
    id: 2,
    title: "آذربایجان غربی",
    slug: "آذربایجان-غربی",
    tel_prefix: "044",
  },
  {
    id: 3,
    title: "اردبیل",
    slug: "اردبیل",
    tel_prefix: "045",
  },
  {
    id: 4,
    title: "اصفهان",
    slug: "اصفهان",
    tel_prefix: "031",
  },
  {
    id: 5,
    title: "البرز",
    slug: "البرز",
    tel_prefix: "026",
  },
  {
    id: 6,
    title: "ایلام",
    slug: "ایلام",
    tel_prefix: "084",
  },
  {
    id: 7,
    title: "بوشهر",
    slug: "بوشهر",
    tel_prefix: "077",
  },
  {
    id: 8,
    title: "تهران",
    slug: "تهران",
    tel_prefix: "021",
  },
  {
    id: 9,
    title: "چهارمحال و بختیاری",
    slug: "چهارمحال-بختیاری",
    tel_prefix: "038",
  },
  {
    id: 10,
    title: "خراسان جنوبی",
    slug: "خراسان-جنوبی",
    tel_prefix: "056",
  },
  {
    id: 11,
    title: "خراسان رضوی",
    slug: "خراسان-رضوی",
    tel_prefix: "051",
  },
  {
    id: 12,
    title: "خراسان شمالی",
    slug: "خراسان-شمالی",
    tel_prefix: "058",
  },
  {
    id: 13,
    title: "خوزستان",
    slug: "خوزستان",
    tel_prefix: "061",
  },
  {
    id: 14,
    title: "زنجان",
    slug: "زنجان",
    tel_prefix: "024",
  },
  {
    id: 15,
    title: "سمنان",
    slug: "سمنان",
    tel_prefix: "023",
  },
  {
    id: 16,
    title: "سیستان و بلوچستان",
    slug: "سیستان-بلوچستان",
    tel_prefix: "054",
  },
  {
    id: 17,
    title: "فارس",
    slug: "فارس",
    tel_prefix: "071",
  },
  {
    id: 18,
    title: "قزوین",
    slug: "قزوین",
    tel_prefix: "028",
  },
  {
    id: 19,
    title: "قم",
    slug: "قم",
    tel_prefix: "025",
  },
  {
    id: 20,
    title: "کردستان",
    slug: "کردستان",
    tel_prefix: "087",
  },
  {
    id: 21,
    title: "کرمان",
    slug: "کرمان",
    tel_prefix: "034",
  },
  {
    id: 22,
    title: "کرمانشاه",
    slug: "کرمانشاه",
    tel_prefix: "083",
  },
  {
    id: 23,
    title: "کهگیلویه و بویراحمد",
    slug: "کهگیلویه-بویراحمد",
    tel_prefix: "074",
  },
  {
    id: 24,
    title: "گلستان",
    slug: "گلستان",
    tel_prefix: "017",
  },
  {
    id: 25,
    title: "لرستان",
    slug: "لرستان",
    tel_prefix: "066",
  },
  {
    id: 26,
    title: "گیلان",
    slug: "گیلان",
    tel_prefix: "013",
  },
  {
    id: 27,
    title: "مازندران",
    slug: "مازندران",
    tel_prefix: "011",
  },
  {
    id: 28,
    title: "مرکزی",
    slug: "مرکزی",
    tel_prefix: "086",
  },
  {
    id: 29,
    title: "هرمزگان",
    slug: "هرمزگان",
    tel_prefix: "076",
  },
  {
    id: 30,
    title: "همدان",
    slug: "همدان",
    tel_prefix: "081",
  },
  {
    id: 31,
    title: "یزد",
    slug: "یزد",
    tel_prefix: "035",
  },
];

export const cards = [
  {
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    isSold: false,
    time: "1402/02/11",
  },
  {
    id: 2,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 10,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "49.500",
    isSold: true,
    time: "1402/8/21",
  },
  {
    id: 3,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 100,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "بار لوکس مخصوص بسته بندی. با شرایط یک ماهه هم فروخته می شود",
    price: "",
    isSold: false,
    time: "1403/02/02",
  },
  {
    id: 4,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 312,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "بار لوکس مخصوص بسته بندی. با شرایط یک ماهه هم فروخته می شود",
    price: "49.500",
    isSold: false,
    time: "1403/04/21",
  },
  {
    id: 5,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 312,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "بار لوکس مخصوص بسته بندی. با شرایط یک ماهه هم فروخته می شود",
    price: "49.500",
    isSold: false,
    time: "1403/04/20",
  },
  {
    id: 5,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 312,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "بار لوکس مخصوص بسته بندی. با شرایط یک ماهه هم فروخته می شود",
    price: "49.500",
    isSold: false,
    time: "1403/06/20",
  },
];

export const trades = [
  {
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    type: "sell",
    time: "1402/12/11",
  },
  {
    id: 2,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    type: "buy",
    time: "1403/03/11",
  },
  {
    id: 3,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    type: "sell",
    time: "1402/05/11",
  },
  {
    id: 4,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    type: "sell",
    time: "1402/05/22",
  },
  {
    id: 4,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    type: "buy",
    time: "1402/06/22",
  },
];

export const transactions = [
  {
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "2300000",
    type: "withdraw",
    time: "1402/12/11",
  },
  {
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "2300000",
    type: "withdraw",
    time: "1402/12/11",
  },
  {
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "2300000",
    type: "withdraw",
    time: "1402/12/11",
  },
  {
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "2300000",
    type: "withdraw",
    time: "1402/12/11",
  },
  {
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "2300000",
    type: "withdraw",
    time: "1402/12/11",
  },{
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "2300000",
    type: "withdraw",
    time: "1402/12/11",
  },{
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "2300000",
    type: "withdraw",
    time: "1402/12/11",
  },{
    id: 1,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "2300000",
    type: "withdraw",
    time: "1402/12/11",
  },
  {
    id: 2,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "450000",
    type: "deposit",
    time: "1403/03/11",
  },
  {
    id: 3,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    type: "withdraw",
    time: "1402/05/11",
  },
  {
    id: 4,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    type: "withdraw",
    time: "1402/05/22",
  },
  {
    id: 4,
    brand: "چکاوک کاشان",
    box: 360,
    weight: 12.5,
    origin: 21,
    badges: ["بدون پرینت", "زرده طلایی", "لوکس"],
    description: "",
    price: "",
    type: "deposit",
    time: "1402/06/22",
  },
];

export const YolkOptions = [
  { title: "زرده ساده", value: "simple" },
  { title: "زرده طلایی", value: "golden" },
  { title: "زرده ذرتی", value: "corn" },
];

export const QualityOptions = [
  { title: "لوکس", value: "lux" },
  { title: "درجه ۱", value: "grade-1" },
  { title: "درجه ۲", value: "grade-2" },
  { title: "کارخانه‌ای", value: "for-factories" },
];

export const PackOptions = [
  { title: "فله (شانه ای)", value: "bulk" },
  { title: "بسته بندی", value: "box" },
];

export const PrintOptions = [
  { title: "بدون پرینت", value: "without" },
  { title: "با پرینت", value: "with" },
  { title: "قابل پرینت", value: "ability" },
];

export const monthNames = [
  "فروردین",
  "اردیبهشت",
  "خرداد",
  "تیر",
  "مرداد",
  "شهریور",
  "مهر",
  "آبان",
  "آذر",
  "دی",
  "بهمن",
  "اسفند",
];
