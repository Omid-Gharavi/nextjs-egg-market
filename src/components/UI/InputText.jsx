...
export default function InputText({
  ...
  useEffect(() => {
    const getProvinces = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_EGG_MARKET}/API/locations/provinces`)
    ...
    const getCities = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_EGG_MARKET_AFARINSESH}/API/locations/area_suggestion`)
      ...
      </label>
      {
        search && <div
          ...
          className={`${inputToggle ? 'visible' : 'invisible'} absolute inset-x-0 top-[100%] bg-default-50 z-20 searchShadow rounded-xl px-4 py-2 h-52 overflow-y-scroll`}>
          {
            ...
            filteredProvinces.map((value, index) => (<div key={index + 1} className="h-10 text-default-900">{value}</div>))
          }
        </div>
      }
      <div className={`relative overflow-hidden border-2 border-[#C2C2C2] rounded-xl`}>
        {
          del &&
          <div
            onClick={() => {
        ...
        <input
          ...
          className={`bg-default-50 ${IR && '!px-9'} w-full px-4 h-[50px] text-default-900 font-normal text-base ${className}`}
          placeholder={placeholder}
        />
      </div>
      {
        IR && <span className="absolute left-4 top-[58%] translate-y-[-55%]">{IR}</span>
      }
      {
        iconRight && <span className={`absolute right-4 text-xl top-[58%] translate-y-[-55%] ${iconRight}`}></span>
      }
      {
        text && <p className="absolute left-4 top-[58%] translate-y-[-55%] text-tertiary text-xs cursor-pointer" onClick={textOnclick}>{text}</p>
      }
    </div>
  );
}
